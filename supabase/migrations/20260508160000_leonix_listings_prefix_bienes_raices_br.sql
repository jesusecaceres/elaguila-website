-- Append `bienes-raices` → BR to `leonix_listings_prefix` (must run after 20260507140000 which redefined the function).
...
comment on function public.leonix_listings_prefix(text) is
  'Maps listings.category to Leonix ad prefix (BR for bienes-raices; TRAV for travel/viajes; COM for comunidad; etc.).';
...
with ordered as (
  ...
  where lower(trim(coalesce(l.category, ''))) = 'bienes-raices'
    and (
...
from (
  ...
  where lower(trim(coalesce(category, ''))) = 'bienes-raices'
    and m is not null
  group by 1
) agg
