-- Append `busco` → BUSCO to `leonix_listings_prefix` (must run after 20260519180000).
...
comment on function public.leonix_listings_prefix(text) is
  'Maps listings.category to Leonix ad prefix (BUSCO for busco; PET for mascotas-y-perdidos; etc.).';

with ordered as (
  select
    l.id,
    'BUSCO'::text as prefix,
  ...
  where lower(trim(coalesce(l.category, ''))) = 'busco'
    and (
...
from (
  ...
  where lower(trim(coalesce(category, ''))) = 'busco'
    and m is not null
  group by 1
) agg
