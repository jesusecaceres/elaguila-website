...
async function fillParentToInventoryStep(page: import("@playwright/test").Page) {
  ...
  await dealerStep.locator("input").first().fill("R30 Proof Dealer");
...
async function assertDraftRelatedCarouselReadOnly(scope: import("@playwright/test").Locator) {
  await expect(scope).toBeVisible();
  await expect(scope.getByText(/Vista previa \/ borrador|Preview \/ draft/).first()).toBeVisible();
  await expect(scope.getByText(/Disponible después de publicar|Available after publishing/).first()).toBeVisible();
  ...
  await expect(scope.getByRole("button", { name: /Ver vehículo|View vehicle/i })).toHaveCount(0);
  await expect(scope.locator("a[href]").filter({ hasText: /Ver detalles|View details/i })).toHaveCount(0);
}

test.describe("A5.RECOVERY-30 draft preview carousel CTA guardrail", () => {
  test.skip(!url || !anon, "Set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in .env.local");

  test("related inventory carousel is read-only in draft preview; Step 7 controls remain", async ({ page, context }) => {
    ...
    await dismissConsentIfPresent(page);
    await expect(page.getByRole("heading", { name: /Más vehículos de este dealer|More vehicles from this dealer/i })).toBeVisible({
      timeout: 60_000,
    });

    const parentRelatedSection = page.locator('[data-autos-related-inventory-preview-only="1"]').first();
    await assertDraftRelatedCarouselReadOnly(parentRelatedSection);
    await expect(parentRelatedSection.locator("a[href^='/']")).toHaveCount(0);
    ...
    const childRelatedSection = page
      .locator('[data-autos-child-inventory-preview="1"]')
      .locator('[data-autos-related-inventory-preview-only="1"]')
      .first();
    await assertDraftRelatedCarouselReadOnly(childRelatedSection);
    await expect(childRelatedSection.locator("a")).toHaveCount(0);
