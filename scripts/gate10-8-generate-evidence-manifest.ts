...
while ((m = rowRe.exec(ledger))) {
  ...
  // Three real table shapes appear in the ledger (confirmed by direct grep of every header row):
  //   5 cells: MD_REF | REQUIREMENT | SOURCE | PROOF | STATUS        (most §0-7, §10-37 sections)
  //   4 cells: REQUIREMENT | FIELD/MECHANISM | PROOF | STATUS        (§8.x subsections)
  //   3 cells: REQUIREMENT | FIELD | STATUS                          (§9 industry-branch subsections)
  // An earlier version of this generator used a fixed rest[0]/rest[1] offset regardless of shape,
  // which put the MD_REF ("§2") into the requirement field and the real requirement text into the
  // source field for every 5-cell row — caught by Gate 10.8B's placeholder-evidence scan and fixed
  // here rather than silently left (400+ rows were affected).
  let requirement: string; let source: string;
  if (rest.length === 5) { requirement = rest[1]; source = rest[2]; }
  else if (rest.length === 4) { requirement = rest[0]; source = rest[1]; }
  else { requirement = rest[0]; source = rest[1]; } // length === 3
  const proofOrField = source;
...
const MECHANISMS: Mechanism[] = [
  ...
  { mechanismId: "M-PROMISE-KEEPER", purpose: "Real commitment bridge, never auto-created", source: ["app/lib/business/projectDiscovery/blueprintCommitmentBridge.ts"], entrypoint: "createCommitmentFromFeedback / createCommitmentFromCheckItem", persistenceObject: "Promise Keeper's own commitment table (createCommitment(), called only from this bridge)", readbackPath: "Promise Keeper's own repository", renderPath: null, authGuard: "requireStaffWorkspaceWriteAccess", negativeTest: "no repository file calls createCommitment() except the dedicated bridge", targetedTests: ["test-project-blueprint-gate7.ts checks 56-58"], liveStagingProof: "Gate 10.8B: real file blueprintCommitmentBridge.ts located and grep-confirmed to define both entrypoints (the manifest's prior citation was an imprecise placeholder path, corrected this pass)" },
