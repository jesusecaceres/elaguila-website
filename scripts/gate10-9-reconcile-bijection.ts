...
const EXPECTED_DELTA: Record<number, { delta: number; reason: string }> = {
  0: { delta: 16, reason: "§0's 18 vision-list items are folded into REQ-0.1 (13-stage flow proof) + REQ-0.2 (anti-pattern guards); giving §0 its own per-item row would double-count the same requirement already proven atomically under §2-§34 (established Gate 10.3)." },
  ...
  37: { delta: 1, reason: "§37's 'The process is repeatable.' and 'The solution is tailored.' combine into REQ-37.2, whose text quotes both; REQ-37.3/37.4 (added this gate) independently cover the two remaining sentences ('quality must represent Leonix' / 'both must win') that had no prior row." },
};
