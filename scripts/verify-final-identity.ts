/**
 * FINAL IDENTITY CLOSEOUT (gate 1) - golden-survivor semantic port.
 *
 * Ported from integration/category-circuit-closeout-2026-09 (a4a1749b4) scripts/verify-final-identity.ts.
...
async function main() {
  ...
  await check("A9 lifecycle wiring (golden): FSBO draft primitive + start-over wipe release the key; NO preview/cancel/retry path does", () => {
  ...
  await check("A10 success return: cleanup component + policy wired to the Revenue OS success page ONLY (not cancel), verified paid returns only", () => {
    ...
    assert.match(page, /category=\{proof\.category \?\? \(category \|\| null\)\}/);
    ...
    assert.match(comp, /if \(!verified \|\| !paid \|\| target !== "real_estate"\) return;/);
  ...
  await check("B2 FSBO hops resolve the same canonical row from SERVER identity: checkout, return, cancel, edit carry the row id", () => {
    const f = raw("app/(site)/clasificados/bienes-raices/preview/privado/components/BienesRaicesPrivadoPreviewClient.tsx");
    ...
    assert.ok(!/publishLeonix|insertListingsRow|\.insert\(/.test(cancelPage), "cancel never creates a row");
    ...
    // golden adaptation: the ONLY insert in the Rentas edit route is the staff Save-for-Client FIRST save (signed
    // custody, linked to the business right after) - an owner dashboard edit still only updates its row.
    assert.equal((rentasEdit.match(/\.insert\(/g) ?? []).length, 1, "one insert site (staff assisted first save)");
    assert.match(rentasEdit, /if \(assisted\.clientUserId\) insertRow\.owner_id = assisted\.clientUserId;[\s\S]{0,200}\.insert\(insertRow\)[\s\S]{0,600}linkAssistedListingToBusiness\(\{/);
  });
