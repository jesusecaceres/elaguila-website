/**
 * Gate R3 — Restore approved Varios stacked detail layout (no experiments)
 * Run: npm run varios:r3-restore-approved-stacked-layout-audit
 */
import fs from "fs";
...
const auditPath = "app/lib/clasificados/en-venta/VARIOS_R3_RESTORE_APPROVED_STACKED_LAYOUT_AUDIT.md";
...
const bundle = [layout, preview, shell, stack, card, gallery, fetchBrowse, publish, contact, engagement].join("\n");
...
add("Approved commit documented", audit.includes("c01156c2"), auditPath);
add("Bad layout commit documented", audit.includes("fc97c454"), auditPath);
add("Tiny desktop root cause documented", audit.includes("max-w-6xl") || audit.includes("90rem"), auditPath);

const requiredAuditRows = [
  "Approved stacked layout commit was identified",
  "Bad layout regression commit was identified",
  "Current tiny desktop root cause was identified",
  "Desktop page is no longer tiny",
...
add("detailViewport token present", brand.includes("detailViewport") && brand.includes("90rem"), "enVentaBrand.ts");
add("Public detail uses detailViewport", layout.includes("EN_VENTA_SURFACE.detailViewport"), "EnVentaAnuncioLayout.tsx");
add("Preview uses detailViewport", preview.includes("EN_VENTA_SURFACE.detailViewport"), "EnVentaPreviewPage.tsx");
...
add("Public gallery object-cover", gallery.includes("object-cover"), "EnVentaMediaGallery.tsx");
...
const gateFiles = diffNames.filter(
  (f) =>
    ...
    f === "scripts/varios-r3-restore-approved-stacked-layout-audit.ts" ||
    f === "package.json"
);
...
add("Gate R3 npm script", pkg.includes("varios:r3-restore-approved-stacked-layout-audit"), "package.json");

const failed = rows.filter((r) => !r.pass);
console.log("# Gate R3 — Restore approved stacked layout audit\n");
for (const r of rows) {
