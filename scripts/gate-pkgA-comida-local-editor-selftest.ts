...
/* 4 — Wiring: registry, dashboard, application, preview. */
{
  ...
  const appCode = app.replace(/\/\/.*$/gm, "");
  assert.ok(!appCode.includes("startRevenueCategoryCheckout") && !appCode.includes("PublishCheckoutCheckpoint"), "the edit form itself must not start or render payment; checkout stays in new-publish preview");
