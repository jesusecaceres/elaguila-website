...
check("04", "Raw internal scan/payment/entitlement enums are not rendered to the owner", () => {
  assert.doesNotMatch(pageSrc, /\{offer\.paymentStatus\} · \{offer\.entitlementStatus\}/);
  ...
  assert.match(pageSrc, /commercialEligibilitySourceLabel\(offer\.commercialEligibilitySource, lang\)/);
  assert.match(pageSrc, /assetLifecycleStatusLabel\(offer\.assetLifecycleStatus, lang\)/);
});
...
check("09", "The rescan CTA is hidden once analysis is complete (shows a completion message instead)", () => {
  ...
  assert.match(manageSrc, /t\.analysisComplete\}.*t\.productsFound\(items\.length\)/s);
  assert.match(manageSrc, /: canScan \? \(/);
});

check("10", "Premature renewal controls are hidden before first publication", () => {
  assert.match(pageSrc, /\{offer\.publishedAt \? \(\s*<div className="mb-6">\s*<OfertasLocalesOwnerRenewalActionCenter/);
  assert.match(
    renewalsSrc,
...
check("11", "A clear first-publication card exists (PUBLICAR ESTA OFERTA / PUBLISH THIS DEAL)", () => {
  ...
  assert.match(pageSrc, /\{offer\.checkoutEligible \? \(/);
});
...
check("16", "Pagar y publicar routes to the dedicated checkout page (not directly to Stripe)", () => {
  ...
  assert.match(pageSrc, /href=\{`\/dashboard\/ofertas-locales\/\$\{offer\.id\}\/checkout\?\$\{q\}`\}/);
