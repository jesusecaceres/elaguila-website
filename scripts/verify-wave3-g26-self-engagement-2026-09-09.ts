...
// Golden re-point (recovery/golden-survivor-integration): the source branch's RENTAS 1-3 asserts
// (RentasListingDetailClient / RentasVisualMatchPreviewView ownerUserId threading) are DEFERRED to
// wave 2 — those files are owned by a concurrent wave. The app/admin/actions.ts REPORT 1 assert is
// dropped too (admin surface is out of scope for this port). Replaced with the shared server-side
// analytics self-engagement short-circuit that this port adds.
{
  const f = read("app/api/analytics/events/route.ts");
  check(
    "ANALYTICS 1: /api/analytics/events short-circuits an owner's own event (selfAction, no write)",
    f.includes("isSelfEngagement(authenticatedUserId, identity.ownerUserId)") && f.includes("selfAction: true"),
  );
  const guardIdx = f.indexOf("isSelfEngagement(authenticatedUserId, identity.ownerUserId)");
  const insertIdx = f.indexOf(".insert(");
  check("ANALYTICS 2: the self-engagement short-circuit runs before any insert", guardIdx > 0 && (insertIdx < 0 || guardIdx < insertIdx));
}
{
  const f = read("app/lib/analytics/selfEngagementGuard.ts");
  check("GUARD DOC: selfEngagementGuard documents reuse for the Report action", f.includes("reuses this for the Report action"));
}
