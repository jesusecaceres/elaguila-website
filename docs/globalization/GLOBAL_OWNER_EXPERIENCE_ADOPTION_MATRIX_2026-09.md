...
**Wave 3 (2026-09-09) — G09-G12, G25-G30 audited (2 parallel research agents); G09/G10, G12,
G26 FIXED.**
...
re-truncated an already-stored phone on every re-edit.
G09/G10 Media: **FIXED.** The shared media engine's `droppedUnpersistable` field (meant so
callers can warn instead of silently losing intent) was never read by any of its ~7 real call
sites — an owner believed N photos saved, only N-1 did, with zero warning. Wired a shared
warning helper at all 11 real call sites (server/console-log level; a user-facing toast is a
separate follow-up needing product copy).
G26 Save/Like/Share/Report: **FIXED.** Rentas' own live detail page mounted Save/Like with no
owner id, so the self-engagement guard never fired there — an owner could save/like their own
Rentas listing. Neither Report implementation checked ownership at all — an owner could report
their own listing. Both fixed.
...
G11 Flyer/Coupon Viewer, G25 Saved Search retry wiring, G29 (Bienes Privado related-listings
gap), G30 (Business Hub consolidation): **real defects found, NOT fixed this pass** — see
ledger section 26 for full detail and fix-size estimates on each.
...
Wave 3 — G09-G12, G25-G30 (public experience globals). AUDITED; G09/G10, G12, G26 FIXED
(2026-09-09, see above); G11/G25/G29(Privado)/G30 real defects recorded, not yet fixed.
