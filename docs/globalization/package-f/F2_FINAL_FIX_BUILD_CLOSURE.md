...
**Migrations applied to any live database:** NONE. One migration file authored (Gate 1), not applied.
...
- **Original defect:** `customerRedemptionCount` was always 0 (never populated by any real caller), so a per-customer-capped promo code could be redeemed unlimited times sequentially by the same person.
- **Files changed:** `app/lib/listingPlans/revenuePromoRedemptions.ts` (new `countCustomerPromoRedemptions()`, wired into `resolvePromoForCheckout()`).
- **Runtime proof:** TypeScript clean; verifier confirms the counter queries `status = 'redeemed'` rows only and reaches `validatePromoEligibility()`.
- **Verifier:** `scripts/verify-package-f-f2-final-fix-build.mjs` (Gate 2 checks).
- **Terminal result:** FIXED + PROVEN for the primary reported defect (unlimited sequential redemption). **NAMED ISOLATED LIMITATION:** true concurrent-race abuse (two simultaneous browser tabs completing payment before either is marked `redeemed`) is not closed — that would require a second DB-level atomic constraint, and only one migration was authorized in this build (spent on P0-1). Disclosed here per the build's explicit "STOP and report exact limitation" instruction rather than silently claiming full race-safety.
