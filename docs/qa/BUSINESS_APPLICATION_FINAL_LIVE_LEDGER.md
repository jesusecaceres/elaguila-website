...
After 5 remediation rounds, all confirmed-RED Part I items with a safe, in-scope source fix have been closed. What remains is exclusively:
...
3. **Genuine owner/browser-only QA** — every item requiring live typing/Spacebar/paste feel, hard refresh, Preview↔Edit click-through, or a real logged-in seller session. Source-side these are all correctly implemented (arrays instead of scalars, dedicated per-group inputs, non-destructive hydration, etc.) but per contract doctrine cannot be certified TRUE without actual browser exercise.
