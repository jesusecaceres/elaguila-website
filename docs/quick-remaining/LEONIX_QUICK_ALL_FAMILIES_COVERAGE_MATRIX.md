...
| **Ofertas Locales** | **C. DIRECT_CANONICAL_LINK (this mission)** | Customer account | Flyer $399 one-time (existing, certain); coupon price disputed across 3 sources (pre-existing defect, not fixed) | Existing form's own media/AI review | `/dashboard/ofertas-locales` | Classified, not modified | `scripts/verify-quick-remaining-families-01.ts` §4, §7 |
