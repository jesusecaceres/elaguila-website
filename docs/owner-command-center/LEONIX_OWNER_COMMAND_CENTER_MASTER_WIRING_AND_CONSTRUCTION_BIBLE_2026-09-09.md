...
| 20 | FINAL FIXED-TERM RENEWAL CONSTRUCTION | PASS — 100%, **committed** | Chuy's decision: build real expiration + same-row renewal for Autos Privado ($24.99/30d) and Bienes Raíces Privado/FSBO ($49.99/45d) — not evergreen. Reused the proven Rentas shared lifecycle engine (`resolveListingLifecycle` + `LISTING_LIFECYCLE_CONFIGS` registry) plus category-specific fulfillment adapters — no new payment system, no duplicate engine. Schema: confirmed `listings.expires_at` already existed (reused directly for FSBO, correcting Gate 19's assumption that both tables lacked it); added one additive migration for `autos_classifieds_listings.expires_at` (nullable column + partial index, deterministic backfill from real `published_at+30d` for existing active rows only) — applied to Leonix Media Staging (the project with real matching QA data for these tables; Production Supabase was never touched; the migration file is committed for the normal release process). Added `AUTOS_PRIVADO_LISTING_LIFECYCLE_CONFIG`/`BR_FSBO_LISTING_LIFECYCLE_CONFIG`, two new checkout-ownership validators, and renewal branches in both categories' webhook fulfillment (same-row only, status never mutated, reused existing locked package keys/prices — no new Stripe price), plus a shared webhook-retry idempotency guard generalized from Rentas' own private copy. Extended public-visibility gating (Bienes' shared browse-eligibility predicate; Autos' pool + direct-by-id fetch) to exclude expired rows from discovery without deleting them, matching Rentas' own established behavior exactly. Discovered and corrected a real mid-gate risk: Autos Privado's actual live dashboard card is `AutosDealerInventoryDashboardSection.tsx`'s `privadoRows` branch, not `mis-anuncios/page.tsx`'s `AutosClassifiedListingManageCard` (legacy/dead code for pre-migration rows) — wired the real card with the same shared `ListingLifecycleStatusCard`/`ListingRenewalAction` components Rentas uses. Wired FSBO's real renewal through `mis-anuncios/page.tsx`'s existing Rentas-only `lifecycle`/`onRenew` plumbing. New verifier (8/8 PASS); all other canonical verifiers re-run clean (2 show fully-explained scope-boundary exceptions — their own guards correctly flagging this gate's authorized migration/BR edits); 0 new TypeScript errors; 0 new lint findings; production build PASS. **CONSTRUCTION: 100% — all 4 items from Gate 18 are now genuinely closed with real, same-row, no-wrong-recharge, webhook-idempotent renewal.** See PROGRESS.md Gate 20 and CABLE_MAP.md's matching section for full detail. |
...
**Chuy's decision on Gate 19's open question: build it — real expiration + same-row renewal for
both categories, evergreen behavior is not acceptable.** Gate 20 (§33.2) implemented exactly that:
reused the proven Rentas shared lifecycle engine, added one additive migration (only
`autos_classifieds_listings` actually needed it — `listings.expires_at` already existed and was
reused directly for FSBO), added both categories' checkout-ownership validators and webhook
renewal branches (same-row, status never mutated, existing locked prices reused, shared
webhook-retry idempotency guard), extended public-visibility gating, and wired the real owner
dashboard renewal CTA for both — catching and correcting a real risk along the way (Autos Privado's
actual live card is `AutosDealerInventoryDashboardSection.tsx`, not the legacy `mis-anuncios`
path a naive implementation would have wired instead). All validation re-run clean.

**FINAL PRODUCT CONSTRUCTION CERTIFICATION: 100% — all 4 items from Gate 18 are now genuinely
closed.** No known source-fixable gaps remain.
...
2. A controlled fast-forward of `main` to the current feature HEAD (see git log for the exact
   latest SHA) and any Production deployment remain a **separate, later gate requiring their own
   explicit authorization** — not automatic from any construction/audit gate having run. Note the
   Gate 20 migration was applied to Leonix Media Staging only — a Production deployment gate must
   separately authorize and apply the equivalent migration to Production Supabase before the
   renewal feature can work there; the migration file is already committed to the repo for that
