...
- **OD-2 — APPROVED.** One additive, reviewed data seed per content batch; never applied by an authoring gate. *(Amended 2026-09-18: there is no staging database. The only target is the canonical project Leonix Media `xuieateniufcrsfdomwl`; "Leonix Media Staging" `cgeehvnfyrdoperdotdh` is retired and prohibited. Exposure option C: live text repairs first (I-1A), new lessons later under their own gate (I-1B).)*
...
- *(Resolved, Gates G4-I1.2 / G4-I1.3.)* The five TODAY-1 English possessive-apostrophe defects (`proteccion_y_datos.summary_en`, `customer_data_protection.summary_en`, and three in `reviews_and_customer_response.body_en`) are repaired by **Part C** of the reviewed I-1 seed — grammar only, guarded, chained.
