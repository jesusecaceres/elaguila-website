-- =====================================================================================================
-- ROLLBACK for 20260920124000_capacity_rpc_commercial_authority.sql - NOT APPLIED.
-- Restores the two PRODUCTION function bodies as observed on 2026-09-18 (logic identical; the pg_get_functiondef md5s
-- recorded then were br_negocio_activate_listing f69cc5331601605c6bfbe4d8fcfdf220 and autos_dealer_activate_listing
-- d5cd8e627012e7c0df7db38fa84e978f - a re-created body may differ by whitespace only, so verify by behaviour).
-- Verify the check is gone (must be 0 for both rows):
--   select proname, position('commercial_authority_required' in pg_get_functiondef(oid)) from pg_proc
--    where proname in ('br_negocio_activate_listing', 'autos_dealer_activate_listing');
-- WARNING: this removes the positive commercial-authority check. The ACL stays service_role-only (that is migration
-- 20260920120000's job and is NOT rolled back here).
-- =====================================================================================================
begin;
