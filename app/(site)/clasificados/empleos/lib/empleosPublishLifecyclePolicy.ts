...
/**
 * D13 / F1 (2026-09 final paid/free circuit audit; recovery port 2026-09-25): the ONE authoritative lane of
 * a publish envelope.
 *
 * The payment decision (`resolveEmpleosUpsertLifecycle`) used to read the TOP-LEVEL `envelope.lane` while the
 * job record / listing content is built from `envelope.payload.lane`, so a logged-in user could send
 ...
 * `envelope.lane` that disagrees is refused (`lane_payload_mismatch`), and a free (feria) lane must carry a
