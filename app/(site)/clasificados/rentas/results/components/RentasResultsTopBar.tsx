...
import { withRentasLandingLang, type RentasLandingLang } from "@/app/(site)/clasificados/rentas/rentasLandingLang";
...
export function RentasResultsTopBar({ copy, lang, routeLang }: Props) {
  const clasificadosHref = withRentasLandingLang("/clasificados", routeLang);
  const rentasHref = withRentasLandingLang(RENTAS_LANDING, routeLang);
  const loginHref = withRentasLandingLang("/login", routeLang);
