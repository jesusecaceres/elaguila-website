...
/** Rentas-only visibility strip — same monetization route, landing/results aligned styling. */
export function RentasLandingVisibilityStrip({ lang }: { lang: Lang }) {
  ...
  const eyebrow = isEs ? "Visibilidad para tu renta" : "Visibility for your rental";

  return (
    <aside className={`${RENTAS_LANDING_SECTION} relative overflow-hidden`} aria-label={label}>
      <div
        ...
        aria-hidden
      />
      <div className={`${RENTAS_LANDING_SECTION_PAD} relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between`}>
