...
import { buildPublishPreviewDisplayStrings } from "@/app/clasificados/lib/publishPreviewStrings";
import {
...
export default function PublicarCategoryApplication() {
  ...
  const [step, setStep] = useState<PublishStep>(() => {
    ...
    if (
      s &&
      (["category", "rentas-track", "basics", "details", "media"] as const).includes(s as PublishStep) &&
  ...
  /** When true, stepΓåÆURL effect should skip (we just pushed in goToStep). */
  ...
  /** After user clicks ΓÇ£SiguienteΓÇ¥, show strong validation styling for that step (publish flow only). */
  ...
  /** Rentas: branches we already ran restore check for (so we donΓÇÖt re-fetch). */
  const checkedRentasBranchesRef = useRef<Set<string>>(new Set());
  /** Rentas: user chose "Start new" for this branch this session; donΓÇÖt show restore modal again. */
  ...
  /** In-app Atr├ís: always go to the real previous step in the flow (setStep + replace URL). Never router.back() so we never leave the publish flow. */
  ...
  // Session gate: redirect to login if not authenticated; fail-safe timeout so "Verificando sesi├│nΓÇª" never hangs.
  ...
  const copy = useMemo(
    () => ({
      es: {
        ...
        subtitle: "Publica con claridad. Mientras m├ís completo, m├ís confianza y mejores resultados.",
        steps: { category: "Categor├¡a", "rentas-track": "Rama", basics: "B├ísicos", details: "Detalles", media: "Media + Contacto + Vista previa" },
        deleteDraft: "Eliminar progreso guardado",
        basicsTitle: "B├ísicos",
        categoryTitle: "Elige la categor├¡a",
        ...
        next: "Siguiente",
        back: "Atr├ís",
        detailsTitle: "Detalles (por categor├¡a)",
        detailsNote:
          "Agrega solo lo que aplica. Estos detalles ayudan a que tu anuncio se vea m├ís profesional.",
        mediaTitle: "Media + Contacto",
        images: "Fotos (m├¡nimo 1)",
        addImages: "Agregar fotos",
        video: "Videos (Pro, hasta 2 por anuncio)",
        addVideo: "Agregar video",
        videoHint: "Hasta 2 videos por anuncio. M├íx 15s, 1080p, ~75MB.",
        rentasPrivadoVideo: "Video (Pro, hasta 1 por anuncio)",
        rentasPrivadoVideoHint: "Hasta 1 video. M├íx 15s, 1080p, ~75MB.",
        ...
        both: "Ambos",
        publish: "Publicar",
        publishing: "Publicando anuncioΓÇª",
        ...
        requiredHint: "Requisitos: Categor├¡a + T├¡tulo + Descripci├│n + Precio/Gratis + Ciudad + 1 foto + Contacto.",
        published: "┬íListo! Tu anuncio fue publicado.",
        viewListing: "Ver anuncio",
        needReqs: "Revisa los requisitos antes de publicar.",
        checking: "Verificando sesi├│nΓÇª",
        todayLabel: "Publicado hoy",
        saveLabel: "Guardar",
        shareLabel: "Compartir",
        contactLabel: "Contactar",
        fullPreviewCta: "Ver versi├│n gratis",
        viewYourListingCta: "Ver tu anuncio",
        fullPreviewTitle: "Vista completa del anuncio",
        fullPreviewBackToEdit: "Volver a editar",
        fullPreviewInfoConfirm: "Confirmo que la informaci├│n es correcta.",
        fullPreviewConfirmPublish: "Confirmar y publicar",
        proPreviewCta: "Ver c├│mo se ver├¡a con Pro",
        proPreviewTitle: "Vista previa Pro",
        proPreviewUpgradeCta: "Mejorar a Pro",
        proPreviewBackToListing: "Volver a mi anuncio",
        proPreviewViewFreeCta: "Ver versi├│n gratis",
        sendMessageLabel: "Enviar mensaje",
        contactHelperText: "As├¡ ver├ín los usuarios c├│mo pueden contactarte.",
        draftInProgress: "Tienes una aplicaci├│n en progreso",
        continueDraft: "Continuar con lo guardado",
        createNewAd: "Crear anuncio nuevo",
        createNewAdHint: "La aplicaci├│n actual se conserva; empezar├ís otro anuncio desde cero.",
        deleteCurrentApplication: "Eliminar aplicaci├│n actual",
        deleteApplication: "Eliminar aplicaci├│n",
        leaveSaveDraft: "Guardar progreso y salir",
        leaveConfirmTitle: "┬┐Salir de la publicaci├│n?",
      ...
      en: {
        title: "Post your ad",
        subtitle: "Post with clarity. The more complete it is, the more trustΓÇöand better results.",
        ...
        detailsNote:
          "WeΓÇÖll add structured category fields in the next batch. For now, we keep the experience clean and safe.",
        ...
        both: "Both",
        publish: "Publish",
        publishing: "Publishing listingΓÇª",
  ...
  // Draft autosave to localStorage (quick resilience) ΓÇö 250ms debounce
  useEffect(() => {
  ...
  // DB autosave ΓÇö immediate for step, category, contactMethod, isFree, images
  useEffect(() => {
  ...
  // DB autosave ΓÇö debounced 500ms for typing-heavy fields
  useEffect(() => {
    if (draftKey === "listing_draft_ssr" || showDraftRestoreModal || !userId) return;
    if (pendingDbSaveRef.current) clearTimeout(pendingDbSaveRef.current);
  ...
  /** Salir: always persist progress (local + DB) then go to Clasificados ΓÇö never clear drafts here. */
...
async function inspectAndThumbVideo(file: File, index: number) {
  ...
  if (file.size > maxBytes) {
    ...
    setVideoErrors((prev) => {
      const n: [string, string] = [...prev];
      n[index] = lang === "es"
        ? "El video es muy grande. Usa un clip m├ís corto o comprimido (m├íx ~75MB)."
  ...
  try {
    ...
    if (info.width > 1920 || info.height > 1080) {
      ...
      setVideoErrors((prev) => {
        const n: [string, string] = [...prev];
        n[index] = lang === "es" ? "El video debe ser 1080p o menos (1920├ù1080)." : "Video must be 1080p or less (1920├ù1080).";
        return n;
      });
      return;
    }
    ...
    setVideoErrors((prev) => {
      const n: [string, string] = [...prev];
      n[index] = lang === "es" ? "No se pudo leer el video. Prueba otro archivo (m├íx 15s, 1080p, ~75MB)." : "Could not read video. Try another file (max 15s, 1080p, ~75MB).";
      return n;
    });
  } finally {
...
async function publish() {
    ...
    if (!publishDraftSnapshot.cityCanonical) {
      setPublishError(lang === "es" ? "Selecciona una ciudad v├ílida del Norte de California." : "Select a valid city in Northern California.");
      return;
    }
    ...
    try {
      ...
      if (!id) {
        setPublishError(lang === "es" ? "Publicado, pero no se recibi├│ ID." : "Published, but no ID returned.");
        return;
      }
...
for (let vi = 0; vi < videoLimit; vi++) {
  ...
  const openFullPreview = useCallback(() => {
    if (typeof window !== "undefined" && process.env.NODE_ENV === "development") {
      // eslint-disable-next-line no-console
      console.log("[publish preview] openFullPreview called", { draftId, step });
    }
    if (userId && typeof performDbSave === "function") {
      void performDbSave();
    }
    fullPreviewModalOpenRef.current = true;
    setFullPreviewVariant(getFullPreviewVariantOnOpen({ isRentasPrivado }));
    setFullPreviewRulesConfirmed(false);
    setFullPreviewInfoConfirmed(false);
    setShowFullPreviewModal(true);
  }, [isRentasPrivado, userId, performDbSave, draftId, step]);

  const handleSharePreview = useCallback(() => {
    if (typeof window === "undefined") return Promise.resolve();
    const url = window.location.href;
    const shareTitle = (title?.trim() || (lang === "es" ? "Vista previa" : "Preview")) + (lang === "es" ? " ΓÇö Leonix" : " ΓÇö Leonix");
    if (navigator.share) {
        ...
        <div className="rounded-2xl border border-black/10 bg-[#F5F5F5] p-6 sm:p-8 shadow-sm">
          ...
          {!checking && signedIn && showFormPlaceholder && (
            <div className="mt-6 rounded-2xl border border-black/10 bg-[#F5F5F5] p-8 text-center text-[#111111]">
              <p className="text-lg font-medium">
                {lang === "es" ? "Formulario disponible pr├│ximamente." : "Form available soon."}
              </p>
              <Link
              ...
              >
                {lang === "es" ? "Publicar en Bienes Ra├¡ces" : "Post in Real Estate"}
              </Link>
            </div>
          )}
          ...
          {!checking && signedIn && !showFormPlaceholder && (
            <>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="draft-restore-title">
                  <div className="rounded-2xl border border-black/10 bg-[#F5F5F5] p-6 max-w-md w-full shadow-xl">
                    ...
                    <p className="mt-2 text-sm text-[#111111]/80">
                      {lang === "es"
                        ? "Puedes continuar con lo guardado, crear otro anuncio nuevo (esta aplicaci├│n se conserva) o eliminar esta aplicaci├│n."
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="leave-confirm-title">
                  <div className="rounded-2xl border border-black/10 bg-[#F5F5F5] p-6 max-w-md w-full shadow-xl">
                    ...
                    <div className="mt-6 flex flex-col gap-3">
                      ...
                      >
                        {leaveSaving ? (lang === "es" ? "GuardandoΓÇª" : "SavingΓÇª") : copy.leaveSaveDraft}
                      </button>
                      <button
                        type="button"
                        disabled={leaveSaving}
              ...
              {/* In-page full preview modal ΓÇö no route, no sessionStorage; uses current form state and ListingView */}
              {showFullPreviewModal && (
                ...
                >
                  ...
                  <div className="sticky top-0 z-10 flex items-center justify-between gap-4 px-4 py-3 border-b border-black/10 bg-[#F5F5F5] shadow-sm">
                    ...
                    >
                      ΓåÉ {copy.fullPreviewBackToEdit}
                    </button>
                    <span className="text-xs text-[#111111]/50">
                      {isRentasPrivado
                        ...
                        : fullPreviewVariant === "pro"
                          ...
                          : lang === "es"
                            ? "Vista previa (como la ver├ín los compradores)"
              ...
              {/* In-page rules modal ΓÇö no navigation, same publish state; z-[120] so it appears above full preview */}
              {showRulesModal && (
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="rules-modal-title">
                  <div className="rounded-2xl border border-black/10 bg-[#F5F5F5] p-6 max-w-lg w-full max-h-[85vh] overflow-y-auto shadow-xl">
                    ...
                    <p className="mt-3 text-sm text-[#111111]/80">
                      {lang === "es"
                        ? "Al publicar en LEONIX Clasificados aceptas que tu anuncio cumple con estas reglas. Esto nos ayuda a mantener un espacio ├║til para todos."
                    ...
                    <ul className="mt-4 space-y-2 text-sm text-[#111111]/90 list-disc list-inside">
                      {(lang === "es"
                        ? [
                            ...
                            "No est├í permitido el spam, contenido enga├▒oso ni duplicados abusivos.",
                            "Respeta a la comunidad: sin contenido ofensivo, discriminatorio o ilegal.",
                            "Los anuncios gratuitos tienen duraci├│n y l├¡mites (por ejemplo 7 d├¡as, 3 fotos). Los planes Pro ofrecen m├ís fotos, video y mayor visibilidad.",
                          ]
                        : [
              ...
              {/* Progress bar ΓÇö Salir moved to category step bottom left */}
              <div className="mt-6">
                <div className="min-w-0 rounded-xl border border-black/10 bg-[#F5F5F5] px-3 py-2.5" role="progressbar" aria-valuenow={currentStepIndex + 1} aria-valuemin={1} aria-valuemax={stepOrder.length} aria-label={lang === "es" ? "Progreso de publicaci├│n" : "Publish progress"}>
                  ...
                  {stepOrder.map((s, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {idx < stepOrder.length - 1 && <span className="mx-1 text-[#111111]/25" aria-hidden>ΓÇ║</span>}
                      </span>
                    );
                  })}
                  </div>
              ...
              {/* Visible checklist from same normalized source as preview/publish ΓÇö pass/fail so form and system stay in sync */}
                ...
                <ul className="flex flex-wrap gap-x-4 gap-y-1.5 text-sm">
                  {requirementItems.map((item) => (
                    <li key={item.key} className={cx("flex items-center gap-1.5", item.ok ? "text-[#111111]/80" : "text-red-700")}>
                      {item.ok ? <span aria-hidden>Γ£ô</span> : <span aria-hidden>Γ£ù</span>}
              ...
              <div className="mt-2 min-h-[1.5rem] text-sm flex items-center" role="status" aria-live="polite">
                {showSaveSuccess && <span className="text-[#0d7a0d] font-medium">Γ£ô {copy.saveProgressSuccess}</span>}
                {saveProgressing && !showSaveSuccess && <span className="text-[#111111]/70">{lang === "es" ? "GuardandoΓÇª" : "SavingΓÇª"}</span>}
              ...
              <div className="mt-6 grid gap-6">
                {/* CATEGORY (STEP 1) ΓÇö icon cards, all real categories, no M├ís */}
                {step === "category" && (
                  <section className="rounded-2xl border border-black/10 bg-[#F5F5F5] p-5">
                    ...
                    {categoryShowValidation && !requirements.categoryOk && (
                      <div className="mt-4 rounded-xl border border-red-300 bg-red-50/90 p-3 text-xs text-red-800" role="alert">
                        {lang === "es" ? "Selecciona una categor├¡a para continuar." : "Choose a category to continue."}
                      </div>
                    )}
                    ...
                    <div ref={categoryActionsRef} className="mt-5 flex flex-wrap items-center gap-3">
                      ...
                      >
                        {saveProgressing ? (lang === "es" ? "GuardandoΓÇª" : "SavingΓÇª") : copy.saveProgress}
                      </button>
                      <button
                        type="button"
                        onClick={() => void handleDeleteCurrentDraft()}
                        ...
                        >
                          <div className="grid-details grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                              <label className="text-sm text-[#111111]">
                                {lang === "es" ? "Subcategor├¡a" : "Subcategory"}{" *"}
                              </label>
                              <select
                              ...
                              >
                                <option value="">{lang === "es" ? "Elige una subcategor├¡aΓÇª" : "Choose oneΓÇª"}</option>
                            ...
                            <div>
                              ...
                              >
                                <option value="">{lang === "es" ? "Elige tipoΓÇª" : "Choose typeΓÇª"}</option>
                          ...
                          <div id="publish-basics-title">
                            ...
                            <p className="mt-1 text-xs text-[#111111]/60">
                              {lang === "es"
                                ? "Un t├¡tulo claro ayuda a que te encuentren."
                                : "A clear title helps people find you."}
                            </p>
                            <input
                              value={title}
                              onChange={(e) => setTitle(e.target.value)}
                              placeholder={lang === "es" ? "Ej: Apartamento 2 rec├ímaras cerca del centro" : "Ex: 2-bed apartment near downtown"}
                              spellCheck
                              autoCorrect="on"
                            ...
                            {!requirements.titleOk && (
                              <div className={cx("mt-1 text-xs", basicsShowValidation ? "text-red-600" : "text-[#111111]/40")}>
                                {lang === "es" ? "M├¡nimo 5 caracteres." : "Min 5 characters."}
                              </div>
                            )}
                          </div>
                          <div id="publish-basics-desc">
                            ...
                            <textarea
                              ...
                              placeholder={
                                lang === "es"
                                  ? "Estado del inmueble, qu├⌐ incluye la renta, reglas, etc."
                          ...
                          <div id="publish-basics-city">
                            ...
                            <CityAutocomplete
                              value={city}
                              onChange={setCity}
                              placeholder={lang === "es" ? "Ej: San Jos├⌐" : "Ex: San Jose"}
                              lang={lang}
                              label=""
                              variant="light"
                              className="mt-2"
                          ...
                          <div>
                            <label className="text-sm text-[#111111]">
                              {lang === "es" ? "Calles principales o direcci├│n aproximada" : "Main streets or approximate address"}
                            </label>
                            <input
                          ...
                          <div>
                            ...
                            >
                              <option value="">{lang === "es" ? "EligeΓÇª" : "ChooseΓÇª"}</option>
                              ...
                              <option value="1-ano">{lang === "es" ? "1 a├▒o" : "1 year"}</option>
                              <option value="2-anos">{lang === "es" ? "2 a├▒os" : "2 years"}</option>
                            ...
                            <>
                              ...
                              <div className="sm:col-span-2">
                                ...
                                <input
                                  ...
                                  placeholder={lang === "es" ? "Ej: Inmobiliaria L├│pez" : "e.g. Lopez Realty"}
                                  className="mt-2 w-full rounded-xl border border-black/10 bg-white/90 px-4 py-3 text-[#111111] placeholder:text-[#111111]/30 focus:outline-none focus:ring-2 focus:ring-yellow-400/30"
                                />
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  placeholder={lang === "es" ? "Ej: Mar├¡a Garc├¡a" : "e.g. Maria Garcia"}
                                  className="mt-2 w-full rounded-xl border border-black/10 bg-white/90 px-4 py-3 text-[#111111] placeholder:text-[#111111]/30 focus:outline-none focus:ring-2 focus:ring-yellow-400/30"
                                />
                              </div>
                              <div>
                              ...
                              <div>
                                <label className="text-sm text-[#111111]">
                                  {lang === "es" ? "Tel├⌐fono de oficina" : "Office phone"}
                                  {details.rentasBranch === "negocio" ? " *" : ""}
                                </label>
                                <input
                                ...
                                {details.rentasBranch === "negocio" && (details.negocioTelOficina ?? "").replace(/\D/g, "").length !== 10 && (details.negocioTelOficina ?? "").length > 0 && (
                                  <div className="mt-1 text-xs text-[#111111]/40">
                                    {lang === "es" ? "Requerido: 10 d├¡gitos." : "Required: 10 digits."}
                                  </div>
                                )}
                              </div>
                              <div>
                              ...
                              <div>
                                ...
                                <div className="mt-2 flex items-center gap-3">
                                  <label className="shrink-0 cursor-pointer rounded-xl border border-[#C9B46A]/50 bg-[#F8F6F0] px-3 py-2 text-xs font-semibold text-[#111111] hover:bg-[#EFE7D8] focus-within:ring-2 focus-within:ring-yellow-400/30">
                                    {logoUploading ? (lang === "es" ? "SubiendoΓÇª" : "UploadingΓÇª") : (lang === "es" ? "Subir imagen" : "Upload image")}
                              ...
                              <div>
                                ...
                                <div className="mt-2 flex items-center gap-3">
                                  <label className="shrink-0 cursor-pointer rounded-xl border border-[#C9B46A]/50 bg-[#F8F6F0] px-3 py-2 text-xs font-semibold text-[#111111] hover:bg-[#EFE7D8] focus-within:ring-2 focus-within:ring-yellow-400/30">
                                    {agentPhotoUploading ? (lang === "es" ? "SubiendoΓÇª" : "UploadingΓÇª") : (lang === "es" ? "Subir imagen" : "Upload image")}
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  placeholder={lang === "es" ? "Ej: Espa├▒ol, ingl├⌐s" : "e.g. Spanish, English"}
                                  className="mt-2 w-full rounded-xl border border-black/10 bg-white/90 px-4 py-3 text-[#111111] placeholder:text-[#111111]/30 focus:outline-none focus:ring-2 focus:ring-yellow-400/30"
                                />
                              </div>
                              <div>
                                ...
                                <input
                                  ...
                                  placeholder={lang === "es" ? "Ej: LunΓÇôVie 9amΓÇô6pm" : "e.g. MonΓÇôFri 9amΓÇô6pm"}
                                  className="mt-2 w-full rounded-xl border border-black/10 bg-white/90 px-4 py-3 text-[#111111] placeholder:text-[#111111]/30 focus:outline-none focus:ring-2 focus:ring-yellow-400/30"
                                />
                              </div>
                              <div>
                              ...
                              {((details.rentasTier ?? "").trim() === "business_plus" || (details.rentasTier ?? "").trim() === "negocio" || (details.bienesRaicesBranch ?? "").trim() === "negocio") && (
                                <div className="sm:col-span-2">
                                  <label className="flex items-center gap-2 text-sm text-[#111111]">
                                    ...
                                    />
                                    {lang === "es" ? "M├ís anuncios de esta empresa" : "More listings from this company"}
                                  </label>
                                </div>
                              )}
                            </>
                        ...
                        <>
                          <div id="publish-basics-title">
                            ...
                            <input
                              value={title}
                              onChange={(e) => setTitle(e.target.value)}
                              placeholder={lang === "es" ? "Ej: Sof├í en excelente condici├│n" : "Ex: Great-condition sofa"}
                              aria-invalid={basicsShowValidation && !requirements.titleOk}
                              className={cx(
                                "mt-2 w-full rounded-xl border bg-white/9 px-4 py-3 text-[#111111] placeholder:text-[#111111]/30 focus:outline-none focus:ring-2 focus:ring-yellow-400/30",
                                basicsShowValidation && !requirements.titleOk ? "border-red-500 ring-1 ring-red-500/35" : "border-black/10"
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div className="sm:col-span-1">
                              ...
                              >
                                {isFree ? (lang === "es" ? "S├¡, es Gratis" : "Yes, it's Free") : lang === "es" ? "No" : "No"}
                              </button>
                            </div>
                          </div>
                ...
                {/* DETAILS ΓÇö only for categories that include details in their step flow (e.g. Rentas). BR has no details step. */}
                    ...
                    <div className="mt-4 rounded-2xl border border-black/10 bg-[#F5F5F5] p-4">
                      ...
                      {getPublishCategoryFields(categoryFromUrl, details).length === 0 ? (
                        <div className="mt-3 text-sm text-[#111111]/55">
                          {lang === "es"
                            ? "Por ahora no hay campos extra para esta categor├¡a."
                            ...
                            if (f.type === "select" && f.options) {
                              return (
                                <label key={f.key} className="block">
                                  ...
                                  >
                                    <option value="">{lang === "es" ? "SeleccionaΓÇª" : "SelectΓÇª"}</option>
                                    {f.options.map((o) => (
                                      <option key={o.value} value={o.value}>
                                        {o.label[lang]}
                                      </option>
                      ...
                      <div className="mt-4 flex items-center justify-between gap-3">
                        ...
                        <div className="text-xs text-[#111111]/40">
                          {lang === "es"
                            ? "Estos detalles se guardan autom├íticamente."
...
<div className="mt-5 flex flex-wrap items-center gap-3">
                    ...
                    <div className="mt-4 grid gap-5">
                      ...
                      />
                        ...
                        {!requirements.imagesOk && (
                          <div className="mt-1 text-xs text-[#111111]/40">
                            {lang === "es" ? "Requerido: m├¡nimo 1 foto." : "Required: at least 1 photo."}
                          </div>
                        )}
                      ...
                      {publishError && (
                        <div
                          ...
                          role="alert"
                        >
                          <div className="font-semibold">Γ¥î {copy.errorTitle}</div>
                      ...
                      <label className="mt-3 flex items-start gap-3 cursor-pointer">
                        ...
                        />
                        <span className="text-sm text-[#111111]">
                          {lang === "es" ? "Confirmo que revis├⌐ mi anuncio y que toda la informaci├│n es correcta." : "I confirm I reviewed my listing and all information is correct."}
                        </span>
                      </label>
              ...
              <div className="mt-6 text-xs text-[#111111]/40 text-center">
                {lang === "es"
                  ? `Sesi├│n: ${userId ? userId.slice(0, 8) + "ΓÇª" : ""} ┬╖ Guardado autom├ítico activo`
                  : `Session: ${userId ? userId.slice(0, 8) + "ΓÇª" : ""} ┬╖ Autosave active`}
              </div>
            </>
          )}
        </div>
          ...
          <div className="w-full max-w-xl rounded-2xl border border-black/10 bg-white shadow-xl">
            <div className="p-5">
              ...
              <div className="mt-1 text-xs text-[#5A5A5A]">
                {lang === "es"
                  ? "Elige tu nivel de presencia digital. No se te cobrar├í hasta publicar."
                  : "Choose your digital presence level. You wonΓÇÖt be charged until you publish."}
              </div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                >
                  ...
                  <ul className="mt-3 space-y-1 text-xs text-[#2B2B2B]">
                    <li>ΓÇó {lang === "es" ? "Logo de su negocio" : "Business logo"}</li>
                    <li>ΓÇó {lang === "es" ? "Perfil b├ísico" : "Basic profile"}</li>
                    <li>ΓÇó {lang === "es" ? "Aparece en b├║squedas y filtros" : "Shows in search & filters"}</li>
                  </ul>
                </button>
                ...
                >
                  ...
                  <ul className="mt-3 space-y-1 text-xs text-[#2B2B2B]">
                    <li>ΓÇó {lang === "es" ? "M├ís fotos + m├ís detalles" : "More photos + more detail"}</li>
                    <li>ΓÇó {lang === "es" ? "Su negocio m├ís visible" : "More visibility"}</li>
                    <li>ΓÇó {lang === "es" ? "Oportunidades mayores para obtener m├ís clientes" : "More chances to win customers"}</li>
                    <li>ΓÇó {lang === "es" ? "Posibilidad de poner videos tambi├⌐n" : "Option to add videos"}</li>
                  </ul>
                </button>
              </div>
