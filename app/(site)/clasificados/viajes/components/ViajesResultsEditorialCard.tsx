  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-dashed border-[color:var(--lx-gold-border)] bg-[color:var(--lx-section)]/90 shadow-[0_10px_28px_-18px_rgba(30,40,55,0.12)]">
      ...
      <div className="flex flex-1 flex-col p-3">
        ...
        <p className="mt-1 line-clamp-2 text-sm text-[color:var(--lx-text-2)]">{row.dek}</p>
        <div className="mt-auto pt-3">
          <Link
            href={href}
            className="flex min-h-[38px] w-full items-center justify-center rounded-xl border border-dashed border-[color:var(--lx-gold)] bg-white/90 px-3 text-sm font-bold text-[color:var(--lx-text)] transition hover:bg-[color:var(--lx-card)]"
          >
            {ui.cards.explore}
          </Link>
        </div>
