...
export function ViajesSectionHeader({ title, subtitle, className = "", showRail = false, eyebrow }: ViajesSectionHeaderProps) {
  return (
    <header className={`text-center sm:text-left ${className}`}>
      <div className={`flex gap-4 ${showRail ? "sm:gap-5" : ""}`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <h2 className="text-[1.35rem] font-bold tracking-tight text-[color:var(--lx-text)] sm:text-2xl md:text-[1.65rem]">{title}</h2>
          {subtitle ? (
