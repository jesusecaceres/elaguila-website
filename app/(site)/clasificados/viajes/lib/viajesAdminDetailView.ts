...
/** Which stored `listing_json` shape the detail was normalized from (golden live rows are V1). */
export type ViajesAdminDetailSourceShape = "v1_negocios" | "v1_privado" | "v2_offer" | "unknown";

export type ViajesAdminDetailSafe = {
  sourceShape: ViajesAdminDetailSourceShape;
  identity: {
    id: string;
    slug: string;
...
/** Classify the stored staged envelope without mutating it. Pure. */
export function detectViajesStagedListingJsonShape(listingJson: unknown): ViajesAdminDetailSourceShape {
  if (!listingJson || typeof listingJson !== "object") return "unknown";
  const root = listingJson as Record<string, unknown>;
  if (root.version === 2 && root.offer && typeof root.offer === "object") return "v2_offer";
  if ((root as { schemaVersion?: unknown }).schemaVersion === 2) return "v2_offer";
  if (root.negocios && typeof root.negocios === "object") return "v1_negocios";
  if (root.privado && typeof root.privado === "object") return "v1_privado";
  return "unknown";
}

/** Build staff-facing detail from a staged row (V1 or V2 via normalize). Pure + read-only. */
export function buildViajesAdminDetailView(row: ViajesStagedListingRow): ViajesAdminDetailSafe {
  const sourceShape = detectViajesStagedListingJsonShape(row.listing_json);
  ...
  return {
    ...
    rawSanitized: {
      version: (row.listing_json as { version?: number } | null)?.version ?? null,
      sourceShape,
      title: row.title,
      lane: row.lane,
      slug: row.slug,
