...
import { useViajesNegociosDraftV2 } from "@/app/(site)/publicar/viajes/negocios/lib/useViajesNegociosDraftV2";
...
export function ViajesNegociosPreviewClient() {
  ...
  const { draft, hydrated } = useViajesNegociosDraftV2(lang === "en" ? "en" : "es");
  ...
  const heroBlobUrl = useViajesLocalHeroObjectUrl("negocios", hero?.localIdbKey ?? null);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        const json = (await res.json()) as { ok?: boolean; row?: { slug?: string; listing_json?: unknown; hero_image_url?: string | null }; error?: string };
        if (!res.ok || !json.ok || !json.row?.listing_json) {
          if (!cancelled) setStagedErr(json.error ?? "load_failed");
          return;
        }
        const offer = normalizeViajesOfferToV2(json.row.listing_json, { locale: lang === "en" ? "en" : "es", laneHint: "business" });
