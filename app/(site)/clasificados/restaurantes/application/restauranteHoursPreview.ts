...
export function computeShellHoursPreview(
  ...
  // Gate G14 — timezone-safe: this used to read now.getDay()/getHours()/getMinutes(), which is
  // the SERVER process's local time (typically UTC), not the restaurant's actual local time —
  // the "Open now" badge could be wrong by several hours. Reuses the timezone-pinned computation
  // already proven correct for the discovery-card "open now" badge (restauranteOpenNowFromHours.ts).
