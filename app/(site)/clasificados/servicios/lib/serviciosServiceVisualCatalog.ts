...
export function emojiVisualGroupForPresetChip(
  ...
  switch (preset.id) {
    ...
    case "jardineria_paisajismo":
      return pair("🌱");
    case "techos_canales":
      if (id === "tech_limpieza" || /limpieza|cleaning|gutter clean/i.test(hay)) return pair("🧹");
      return pair("🏠");
    ...
    case "servicios_impresion":
      return pair("🖨️");
    case "tramites_gestoria":
      if (/legal|ley |attorney|abogad/i.test(hay)) return pair("⚖️");
      if (/licencia|license|permiso|permit/i.test(hay)) return pair("🪪");
      return pair("📋");
    ...
    case "servicio_otro_generico":
      ...
      if (/domicilio|on-site|at home|a domicilio/i.test(hay)) return pair("🏠");
  ...
  if (id.startsWith("alm_")) return pair("📦");
...
/** Keyword rows: first match wins (normalized haystack). Exported for preset smoke audits. */
export const SERVICIOS_SERVICE_LABEL_KEYWORD_RULES: { test: RegExp; emoji: string; visualGroup: string }[] = [
  { test: /iphone|android|celular|mobile phone|smartphone|tablet|pantalla rota|reparacion de celular/i, emoji: "📱", visualGroup: "technology_support" },
  { test: /computadora|laptop|pc |macbook|reparacion de pc/i, emoji: "💻", visualGroup: "technology_support" },
  { test: /limpieza|cleaning|oficina|janitorial|aseo|limpieza de casa/i, emoji: "🧹", visualGroup: "home_trade" },
  { test: /plomer|fugas|drenaje|fontan|water heater|calentador/i, emoji: "🔧", visualGroup: "home_trade" },
  { test: /electric|electricidad|ilumin|lighting|cableado/i, emoji: "⚡", visualGroup: "home_trade" },
  ...
  { test: /mecan|automot|llanta|transmission|grua|towing|oil change|cambio de aceite|aceite motor/i, emoji: "🚗", visualGroup: "automotive" },
  { test: /barber|peluquer|corte de cabello|haircut|hair color/i, emoji: "✂️", visualGroup: "health_beauty" },
  { test: /(^unas$)|manicur|pedicur|\bnails?\b|uñas|unas acril|gelish|esmaltado/i, emoji: "💅", visualGroup: "health_beauty" },
  ...
  { test: /abogad|notar|legal |ley |attorney|inmigr|immigration|visa\b|ciudadan/i, emoji: "⚖️", visualGroup: "legal_professional" },
  ...
  { test: /mariachi/i, emoji: "🎺", visualGroup: "events_entertainment" },
  { test: /quinceañera|quinceanera|xv años|xv anos/i, emoji: "👑", visualGroup: "events_entertainment" },
  { test: /banda en vivo|live band|grupo regional|regional music/i, emoji: "🎸", visualGroup: "events_entertainment" },
  { test: /solista|solo musician|músico para evento|musico para evento/i, emoji: "🎤", visualGroup: "events_entertainment" },
  { test: /sonido e ilumin|sound (&|and) lighting|equipo de sonido|pa system|dj\b/i, emoji: "🔊", visualGroup: "events_entertainment" },
  { test: /animador|entertainer|show en vivo|live music/i, emoji: "🎶", visualGroup: "events_entertainment" },
  ...
  { test: /web |app |software|desarrollo|developer|desarrollo web|it |redes|network/i, emoji: "💻", visualGroup: "technology_support" },
