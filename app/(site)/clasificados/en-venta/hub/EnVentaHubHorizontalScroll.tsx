...
const railScrollClass =
  "flex snap-x snap-mandatory gap-2 overflow-x-auto scroll-pl-3 scroll-pr-8 pb-2 pt-0.5 [-webkit-overflow-scrolling:touch] [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden";
...
export function EnVentaHubSwipeHintBadge({ label }: { label: string }) {
  return (
    <span
      className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-[#C9A84A]/45 bg-gradient-to-r from-[#FFFBF0] to-[#FFFDF7] px-2.5 py-1 text-[11px] font-semibold text-[#8A6B1F] shadow-[0_2px_8px_-4px_rgba(201,168,74,0.35)] sm:hidden"
    >
...
export function EnVentaHubScrollEdgeFade({ fromClass = "from-[#FAF6EE]" }: { fromClass?: string }) {
  return (
    <div
      className={`pointer-events-none absolute inset-y-0 right-0 z-[1] w-10 bg-gradient-to-l ${fromClass} via-60% to-transparent sm:hidden`}
      aria-hidden
    />
  );
}
...
function EnVentaHubScrollRailDots() {
  return (
    <div className="mt-1 flex justify-center gap-1 sm:hidden" aria-hidden>
      <span className="h-1 w-6 rounded-full bg-[#C9A84A]/55" />
      <span className="h-1 w-1 rounded-full bg-[#D6C7AD]/90" />
      <span className="h-1 w-1 rounded-full bg-[#D6C7AD]/70" />
    </div>
  );
}
...
export function EnVentaHubMobileScrollRail({
  ...
  return (
    <div className="relative min-w-0 -mx-1 sm:mx-0">
      ...
      {!mobileOnly ? <EnVentaHubScrollEdgeFade fromClass={edgeFadeFromClass} /> : null}
      {showRailDots && !mobileOnly ? <EnVentaHubScrollRailDots /> : null}
      {mobileOnly ? (
        <>
          <EnVentaHubScrollEdgeFade fromClass={edgeFadeFromClass} />
