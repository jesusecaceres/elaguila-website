/**
 * Wave 4 P0 fix — single source of truth for parsing a published Bienes Raíces Negocio
 * (agente-individual) row's `business_meta`/`detail_pairs` back into the full
 * `AgenteIndividualResidencialFormState` shape.
 *
 * Extracted verbatim from `BienesRaicesNegocioLiveDetailShell.tsx`'s `buildPublishedState()` —
 * the function that already correctly renders every one of these fields on the real live public
 * page. The dashboard-edit reverse mapper (`bienesPublishedToAgenteApplicationDraft.ts`) was a
 * separate, much thinner reimplementation that silently dropped ~130 of ~150 form fields on
 * every edit (confirmed destructive on Republish). Both the public shell and the reverse mapper
 * now call this one function, so there is exactly one place that knows how to read this data.
 */
import {
...
export type BienesAgenteResidencialPublishedRowLike = {
  id: string;
...
export type BienesAgenteResidencialParentIdentityRowLike = {
  id: string;
  business_name?: string | null;
  business_meta?: string | null;
  contact_phone?: string | null;
...
export function parseBienesAgenteResidencialPublishedState(input: {
  listing: BienesAgenteResidencialPublishedRowLike;
  parentIdentity?: BienesAgenteResidencialParentIdentityRowLike | null;
  lang: "es" | "en";
  ...
  return {
    ...
    // Gate G23 — verification metadata has no dedicated detail_pairs key today; the plain street
    // text above already round-trips, so this defaults to "manual" rather than fabricating a
    // provider/verified state that was never recorded.
    direccionVerificationStatus: trim(gate.streetAddress) ? "manual" : base.direccionVerificationStatus,
    direccionProvider: base.direccionProvider,
    direccionProviderPlaceId: base.direccionProviderPlaceId,
