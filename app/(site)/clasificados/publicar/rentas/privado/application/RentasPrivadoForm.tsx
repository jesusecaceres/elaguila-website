...
import { loadRentasPrivadoDraft, saveRentasPrivadoDraft } from "./utils/rentasPrivadoDraft";
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#F6F0E2] px-4 pb-28 pt-24 text-[#2C2416] sm:px-5 sm:pb-24 sm:pt-28">
      <div className="mx-auto w-full min-w-0 max-w-3xl space-y-7 md:space-y-8">
        <header className="min-w-0">
          <p className="text-xs font-bold uppercase tracking-wide text-[#B8954A]">Leonix · Rentas · Privado</p>
          <h1 className="mt-2 text-2xl font-extrabold tracking-tight text-[#1E1810] sm:text-[1.65rem]">Publicar renta — Particular</h1>
          ...
          <div className="mt-4 grid min-w-0 gap-4 sm:grid-cols-2 sm:gap-5">
            ...
            <AiField label="Plazo del contrato">
              <select
                ...
                onChange={(e) =>
                  setState((s) => ({
                    ...s,
                    plazoContrato: e.target.value as RentasPrivadoFormState["plazoContrato"],
                  }))
                }
              >
                <option value="">—</option>
            ...
            <AiField label="Amueblado">
              <select
                ...
                onChange={(e) =>
                  setState((s) => ({ ...s, amueblado: e.target.value as RentasPrivadoFormState["amueblado"] }))
                }
              >
                <option value="">—</option>
            ...
            <AiField label="Mascotas">
              <select
                ...
                onChange={(e) =>
                  setState((s) => ({ ...s, mascotas: e.target.value as RentasPrivadoFormState["mascotas"] }))
                }
              >
                <option value="">—</option>
            ...
            <AiField label="Estado del anuncio">
              <select
                ...
                onChange={(e) =>
                  setState((s) => ({ ...s, estadoAnuncio: e.target.value as RentasPrivadoFormState["estadoAnuncio"] }))
                }
              >
                {ESTADOS.map((o) => (
        ...
        <p className="break-words text-center text-xs leading-relaxed text-[#5C5346]/80">
          ...
          <code className="break-all rounded bg-[#F9F6F1] px-1.5 py-0.5 text-[11px]">{RENTAS_PUBLICAR_PRIVADO_PUBLIC_ENTRY}</code>
        </p>
      </div>
    </main>
  );
