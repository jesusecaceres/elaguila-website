...
export function CategoryStandardSearchRow({
  ...
  return (
    <form
      action={action}
      method={method}
      className={`rounded-xl border border-[#D6C7AD] bg-[#FFFDF7] p-4 shadow-[0_6px_24px_-16px_rgba(31,36,28,0.12)] sm:p-5 ${className}`.trim()}
    >
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-stretch">
        <label className="min-w-0 flex-1">
          ...
          <input
            ...
            className="min-h-[2.75rem] w-full rounded-lg border border-[#D6C7AD] bg-white px-3 text-sm text-[#1F241C] outline-none placeholder:text-[#3D3428]/50 focus:border-[#C9A84A]/70 focus:ring-2 focus:ring-[#C9A84A]/20"
          />
        </label>
        ...
        <button
          type="submit"
          className="inline-flex min-h-[2.75rem] shrink-0 items-center justify-center rounded-lg bg-[#7A1E2C] px-6 text-sm font-bold text-[#FFFDF7] transition hover:bg-[#5e1721] lg:min-w-[7.5rem]"
        >
          {btn}
        </button>
      </div>
