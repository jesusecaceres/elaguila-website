...
import {
  autosDealerInventoryDrawerContinueCta,
...
import { autosDealerInventoryAddTenSlotsCta } from "@/app/lib/clasificados/autos/autosDealerInventoryValueCopy";
import {
...
export function AutosNegociosInventoryValueDrawer({ open, onClose, lang, addCtx, counts }: Props) {
  ...
  const handlePrimary = () => {
    if (atLimit) {
      window.location.href = autosDealerInventoryUpgradeContactHref(lang);
      return;
    }
    onClose();
  ...
  const primaryLabel = atLimit
    ? autosDealerInventoryRequestBoostCta(lang)
    : autosDealerInventoryDrawerContinueCta(lang);
