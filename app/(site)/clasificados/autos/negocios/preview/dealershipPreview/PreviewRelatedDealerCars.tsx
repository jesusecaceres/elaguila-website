...
import { PreviewAutosDealerInventoryVehicleCard } from "./PreviewAutosDealerInventoryVehicleCard";
import {
...
export function PreviewRelatedDealerCars({
  listings,
  fullInventoryHref,
  hasMore,
  previewOnly = false,
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p className={autosPreviewSectionEyebrowClass}>
            {lang === "es" ? "Inventario del negocio" : "Dealer inventory"}
          </p>
          <h2 className={`mt-1 ${autosPreviewSectionTitleClass}`}>
            {lang === "es" ? "Más autos de este negocio" : title}
          </h2>
      ...
      <div className={`mt-5 ${autosRelatedInventoryShelfScrollClass}`}>
        {visible.map((car) => (
          <div key={car.id} className={autosRelatedInventoryShelfCardShellClass}>
            <PreviewAutosDealerInventoryVehicleCard
              car={{
                id: car.id,
