...
import { formatMiles, formatMpgPair, formatStockDisplay, formatVinDisplay } from "../../components/autoDealerFormatters";
...
const CARD = `${autosPreviewPremiumCardClass} p-4 sm:p-5`;

export function PreviewVehicleSpecsGrid({
  data,
  hiddenRowKeys,
  variant = "full",
}: {
  ...
  const CANVAS_STRIP_KEYS = ["trans", "drive", "eng", "fuel", "body"] as const;
  const stripVisible =
    variant === "canvasStrip"
  ...
  return (
    <section className={CARD}>
      ...
      <div className="mt-4 hidden min-w-0 auto-rows-fr gap-2.5 sm:grid sm:grid-cols-2 lg:grid-cols-3">
        {visible.map((r) => (
          <SpecIconRow
            key={r.key}
            icon={r.icon}
