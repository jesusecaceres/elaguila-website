...
/** Decorative faux map — no external tiles or geolocation claims (A5.QA-01). */
export function AutosNegociosBusinessHubFauxMap() {
  const uid = useId().replace(/:/g, "");
  const gridId = `autos-hub-map-grid-${uid}`;
  const washId = `autos-hub-map-wash-${uid}`;
  const vignetteId = `autos-hub-map-vig-${uid}`;

  return (
    <div
      className="relative aspect-[16/9] w-full max-w-full overflow-hidden rounded-xl"
      style={{
