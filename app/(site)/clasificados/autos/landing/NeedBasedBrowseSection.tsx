      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {cards.map((c) => (
          ...
          >
            ...
            <div className="mt-4 flex items-center justify-end gap-1 text-[11px] font-bold uppercase tracking-wide text-[color:var(--lx-gold)] sm:text-xs">
              <span className="text-balance text-right">{copy.needBrowseApply}</span>
              <FiArrowUpRight className="h-4 w-4 shrink-0 transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" aria-hidden />
            </div>
          </Link>
        ))}
      </div>
