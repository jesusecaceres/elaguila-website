...
export function BienesRaicesResultsTopBar({ copy, lang }: { copy: BrResultsCopy; lang: Lang }) {
  const loginHref = appendLangToPath("/login", lang);
  return (
    <header className="mb-6 flex flex-col gap-4 border-b border-[#E8DFD0]/80 pb-5 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex flex-wrap items-center gap-3 sm:gap-4">
        <Link
          href={appendLangToPath("/clasificados", lang)}
        ...
        <nav className="flex flex-wrap items-center gap-2 text-[11px] font-bold uppercase tracking-[0.12em] text-[#5C5346]">
          <Link href={appendLangToPath(BR_CATEGORY_HOME, lang)} className="hover:text-[#B8954A]">
            {copy.breadcrumbCategory}
          </Link>
          <span className="text-[#C9B46A]" aria-hidden>
            ›
          </span>
          <span className="text-[#5C5346]">{copy.breadcrumbResults}</span>
          <span className="text-[#C9B46A]" aria-hidden>
            ›
          </span>
