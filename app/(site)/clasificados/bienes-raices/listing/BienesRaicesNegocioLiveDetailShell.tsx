...
/**
 * Gate BIENES-NEGOCIO-1 — this shell no longer owns the published-row -> form-state
 * interpretation. `buildPublishedState()` and its ~9 helpers moved VERBATIM into the shared pure
 * module below, which the dashboard-edit reverse mapper now calls too. One module, one
 * interpretation: the public page and the owner's edit form can no longer disagree about what a
 * published Bienes Negocio row means. See that module's header for the sealed-`733408dd`
 * forward-port record.
 */
import {
...
export function BienesRaicesNegocioLiveDetailShell({
  ...
  const data = useMemo(() => parseBienesAgenteResidencialPublishedState({ listing, parentIdentity, lang }), [lang, listing, parentIdentity]);
