...
export function DashboardCategoryListingCard({
  ...
  return (
    <article className="rounded-3xl border border-[color:var(--lx-border)]/70 bg-[color:var(--lx-card)] p-5 shadow-[0_10px_32px_-12px_rgba(42,36,22,0.08)]">
      <div className="flex flex-wrap items-center gap-2">
        <span className="rounded-full bg-[color:var(--lx-section)] px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-[color:var(--lx-text-2)]">
          {categoryLabel}
        </span>
        <span className="rounded-full bg-emerald-100 px-2.5 py-0.5 text-[11px] font-bold text-emerald-900">{status}</span>
        {badges.map((badge) => (
          <span key={badge} className="rounded-full border border-[color:var(--lx-border)] px-2 py-0.5 text-[11px] font-semibold text-[color:var(--lx-muted)]">
            {badge}
          </span>
        ))}
      </div>
      <h3 className="mt-3 text-lg font-bold text-[color:var(--lx-text)]">{title}</h3>
      {subtitle ? <p className="mt-1 text-sm text-[color:var(--lx-muted)]/90">{subtitle}</p> : null}
      {metaItems.length > 0 ? (
      ...
      {footerHint ? <p className="mt-3 text-[10px] leading-snug text-[color:var(--lx-muted)]/95">{footerHint}</p> : null}
      <div className="mt-4">
