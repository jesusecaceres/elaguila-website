...
export function TiendaMarketingMaterialsShowcase(props: { lang: Lang }) {
  ...
  return (
    <section className="mt-8 overflow-hidden rounded-2xl border border-[color:var(--lx-border)] bg-[color:var(--lx-section)] p-4 sm:p-6 shadow-[0_20px_60px_rgba(42,36,22,0.08)]">
      <h2 className="text-sm font-semibold tracking-tight text-[color:var(--lx-text)]">
        {lang === "en" ? "What we produce in this lane" : "Qué producimos en esta categoría"}
      </h2>
      <p className="mt-1 max-w-3xl text-xs text-[color:var(--lx-muted)] sm:text-sm">
        {lang === "en"
      ...
      <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3 lg:grid-cols-4">
        {marketingMaterialsShowcaseItems.map((item, i) => {
          ...
          return (
            <div
              key={`${item.label.en}-${i}`}
              className="group relative overflow-hidden rounded-xl border border-[color:var(--lx-border)] bg-[color:var(--lx-canvas)]"
            >
