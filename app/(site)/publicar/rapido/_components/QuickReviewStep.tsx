...
import { quickCard, quickPrimaryBtn } from "./QuickShell";

type Props = {
  lang: QuickLang;
  definition: QuickClassifiedDefinition;
  steps: readonly QuickIntakeStep[];
  values: QuickIntakeValues;
  media: QuickMediaItem[];
  confirmations: QuickConfirmations;
  surface: QuickConfirmationSurface;
  submitting: boolean;
  issues: string[];
...
/** Price is display-only and always read from the server-side pricing authority at render time. */
function priceLine(def: QuickClassifiedDefinition, lang: QuickLang): string | null {
  if (def.pricing.kind !== "paid") return null;
  const { priceCents, definition } = getRevenuePackagePriceCents({ category: def.pricing.category, packageKey: def.pricing.packageKey });
  if (priceCents == null) return null;
  const days = definition?.durationDays;
  const base = formatRevenuePriceLabel(priceCents);
  if (!days) return base;
  return lang === "en" ? `${base} · ${days} days` : `${base} · ${days} días`;
}

export function QuickReviewStep({ lang, definition, steps, values, media, confirmations, surface, submitting, issues, onEditStep, onConfirmations, onSubmit }: Props) {
    ...
    <div className="space-y-4">
      ...
      {surface.kind === "community" ? (
        <div className={quickCard}>
          <CommunityPublishConfirmationSection
            ...
            value={confirmations}
            onChange={(patch) => onConfirmations({ ...confirmations, ...patch })}
          />
        </div>
      ) : null}
      ...
      <section className={quickCard}>
        <p className="text-sm font-semibold">
          {price ? `${quickCopy("reviewPaidNote", lang)} ${price}` : quickCopy("reviewFreeNote", lang)}
        </p>
        <p className="mt-1 text-xs text-[#7A7164]">{quickCopy("reviewHandoffNote", lang)}</p>
        {issues.length ? (
