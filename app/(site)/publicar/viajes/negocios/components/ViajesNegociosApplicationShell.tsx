...
import { useViajesNegociosDraftV2 } from "../lib/useViajesNegociosDraftV2";
import { ViajesNegociosStepGetaway } from "./ViajesNegociosStepGetaway";
import { ViajesNegociosStepInclusions } from "./ViajesNegociosStepInclusions";
import { ViajesNegociosStepMedia } from "./ViajesNegociosStepMedia";
import { ViajesNegociosStepBusinessHub } from "./ViajesNegociosStepBusinessHub";
import { ViajesNegociosStepReview } from "./ViajesNegociosStepReview";
import {
...
export function ViajesNegociosApplicationShell() {
  ...
  const { draft, setOffer, hydrated, setDraft } = useViajesNegociosDraftV2(lang === "en" ? "en" : "es");
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const next = normalizeViajesOfferToV2(json.row.listing_json, { locale: lang === "en" ? "en" : "es", laneHint: "business" });
  ...
  const steps = useMemo(
    () =>
      lang === "en"
        ? ["The getaway", "What travelers get", "Show the experience", "Business & contact", "Review & publish"]
        : ["La escapada", "Qué reciben", "Muestra la experiencia", "Negocio y contacto", "Revisar y enviar"],
    [lang]
  );

  const onChangeOffer = useCallback((next: ViajesOfferModelV2) => setOffer(next), [setOffer]);
  ...
  async function onSubmit() {
    ...
    if (issues.length) {
      ...
      setStep(issues[0]?.step ? issues[0].step - 1 : 0);
      return;
    }
    setSubmitBusy(true);
    try {
      ...
      const respLane = (json.lane || "business").trim() || "business";
      if (!id || !slug) {
    ...
    >
      ...
      <div className="mx-auto max-w-3xl px-4 pb-8 pt-2 sm:px-6 sm:pt-4">
        ...
        <div className={`${CARD} mt-4`}>
          {step === 0 ? <ViajesNegociosStepGetaway offer={offer} onChange={onChangeOffer} lang={lang === "en" ? "en" : "es"} /> : null}
          {step === 1 ? <ViajesNegociosStepInclusions offer={offer} onChange={onChangeOffer} lang={lang === "en" ? "en" : "es"} /> : null}
          {step === 2 ? (
            <ViajesNegociosStepMedia
              offer={offer}
              onChange={onChangeOffer}
              lang={lang === "en" ? "en" : "es"}
              draftId={offer.id || stagedIdFromUrl || "negocios-draft"}
              getBearerToken={getBearerToken}
            />
          ) : null}
          {step === 3 ? <ViajesNegociosStepBusinessHub offer={offer} onChange={onChangeOffer} lang={lang === "en" ? "en" : "es"} /> : null}
          {step === 4 ? (
            <ViajesNegociosStepReview
              offer={offer}
              onChange={onChangeOffer}
              lang={lang === "en" ? "en" : "es"}
              onGoStep={setStep}
