...
export function AutosNegociosMediaManager({
  ...
  const existingImageUrls = useCallback(() => {
    return new Set(images.map((x) => x.url.trim().toLowerCase()).filter(Boolean));
  }, [images]);
  ...
  const addSingleImageUrl = (): boolean => {
    ...
    if (!result.ok) {
      setSingleUrlError(
        result.reason === "video" ? m.videoUrlRejected : m.invalidImageUrl,
      );
      return false;
    }
    if (existingImageUrls().has(result.url.toLowerCase())) {
      setSingleUrlError(lang ? autosExternalImageUrlDuplicate(lang) : m.duplicateImageUrl);
      return false;
    }
    ...
    const addition: MediaImageEntry = {
      ...
      url: result.url,
      sourceType: "url",
      isPrimary: base.length === 0,
      sortOrder: base.length,
    };
    const merged = normalizeMediaImagesOrder([...base, addition]);
    commitImages(ensureOnePrimaryMedia(merged));
  ...
  const remove = (id: string) => {
    ...
    commitImages(ensureOnePrimaryMedia(normalizeMediaImagesOrder(next)));
  };
  ...
  const move = (id: string, dir: -1 | 1) => {
    ...
    const a = sorted[idx]!;
    const b = sorted[j]!;
    const next = (listing.mediaImages ?? []).map((x) => {
  ...
  const urlPlaceholder = m.imageUrlPlaceholder;
