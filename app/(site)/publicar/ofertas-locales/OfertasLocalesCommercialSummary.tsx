...
export function OfertasLocalesCommercialSummary({ draft, lang }: Props) {
  ...
  const durationNote = useMemo(
    () => (product?.aiIncluded ? `${c.step7Duration} · ${c.step7AiIncludedNote}` : c.step7Duration),
    [c.step7AiIncludedNote, c.step7Duration, product?.aiIncluded]
  );
      ...
      <div className="mt-3 space-y-2 text-sm">
        <div className="flex items-baseline justify-between gap-3">
          ...
          <span className="text-lg font-bold text-[#7A1E2C]">
            {isFreeProduct ? c.freeLabel : formatMoney(baseCents)}
          </span>
        </div>
