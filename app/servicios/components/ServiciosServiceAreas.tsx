...
import { showMainServiceAreasSection } from "../lib/serviciosProfileVisibility";
import { SV } from "./serviciosDesignTokens";

/** Main column: city / area list only (map lives in the sticky sidebar when present). */
export function ServiciosServiceAreas({ profile, lang }: { profile: ServiciosBusinessProfile; lang: ServiciosLang }) {
  const L = getServiciosProfileLabels(lang);
  const areas = profile.serviceAreas;
  if (!showMainServiceAreasSection(profile)) return null;
