...
export default function ClasificadosListaPage() {
  ...
  const lang: Lang = (getParam("lang") as Lang) === "en" ? "en" : "es";
  ...
  // Local state mirrors params (controlled inputs)
  ...
  // Keep state in sync if params change
  useEffect(() => {
    setQ(qParam);
  ...
  const filtered = useMemo(() => {
    ...
    const wantLatLng = (() => {
      ...
      // ✅ CityRecord field is `city` (not `name`)
    ...
    const withinRadius = (listingCity: string) => {
      ...
      // ✅ CityRecord field is `city` (not `name`)
      const c = CA_CITIES.find((x) => normalize(x.city) === key);
      if (!c) return true; // unknown city: don't block
