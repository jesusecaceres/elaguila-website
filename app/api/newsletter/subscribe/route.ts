...
export async function POST(req: Request) {
  ...
  // Newsletter promo code minting (Launch 25) has been retired. Subscription
  // still succeeds; these fields are kept in the response shape for existing
  // clients but always report "no promo code" now.
  const promoCodeCreated = false;
  const promoCodeReused = false;
  const promoCodeEmailSent = false;
  const promoCodeEmailStatus:
    | "sent"
    | "failed"
    | "not_configured"
    | "not_created"
