...
import {
  applyMediaKitLifecycleAdmin,
  updateMediaKitLeadNotesAdmin,
...
export async function PATCH(req: Request, context: RouteContext) {
  ...
  if (actionRaw === "archive" || actionRaw === "restore" || actionRaw === "delete") {
    const result = await applyMediaKitLifecycleAdmin(id, actionRaw as "archive" | "restore" | "delete");
    if (!result.ok) {
      const status =
  ...
  if (o.internal_notes != null) {
    const result = await updateMediaKitLeadNotesAdmin(id, String(o.internal_notes));
    if (!result.ok) {
