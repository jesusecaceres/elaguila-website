...
import { normalizeDocumentAiResultToOfertaLocalItems } from "@/app/lib/ofertas-locales/ofertasLocalesAiNormalizer";
import {
  mapOfertaLocalSearchableItemDraftToDbInsert,
  mapOfertaLocalScanJobRecordDraftToDbInsert,
} from "@/app/lib/ofertas-locales/ofertasLocalesAiDbMapper";
import {
...
} from "@/app/lib/ofertas-locales/ofertasLocalesDocumentAiClient";
import type {
...
function isDbTableMissingError(message: string | undefined): boolean {
  ...
  return (
    m.includes("oferta_local_scan_jobs") &&
...
export async function POST(req: NextRequest) {
  ...
  const { data: parentOffer, error: parentError } = await supabase
    ...
    .select("id, owner_id, business_name, address, city, state, zip_code, valid_from, valid_until")
  ...
  if (parentError) {
    ...
    return NextResponse.json<OfertaLocalScanApiResponse>(
      { ok: false, error: "parent_lookup_failed", detail: parentError.message, configurationMissing: false },
      { status: 500 }
    );
  }

  if (!parentOffer || parentOffer.owner_id !== ownerId) {
    return NextResponse.json<OfertaLocalScanApiResponse>(
      { ok: false, error: "forbidden", detail: "Offer not found or not owned by you.", configurationMissing: false },
      { status: 403 }
    );
  }
