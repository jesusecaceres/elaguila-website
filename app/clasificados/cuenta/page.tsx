  ...
  if (checking) {
    return (
      <div className="min-h-screen bg-[#D9D9D9] text-[#111111]">
        <Navbar />
        <div className="mx-auto max-w-4xl px-4 py-16 text-[#111111]">…</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#D9D9D9] text-[#111111]">
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        <div className="mb-6">
          ...
          <div className="mt-2 text-sm text-[#111111]">{t.note}</div>
          <div className="mt-4">
            <Link href={`/clasificados?lang=${lang}`} className="text-sm text-[#111111] underline underline-offset-4">
              {t.back}
            </Link>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-black/10 bg-[#F5F5F5] p-6">
          <div className="text-sm font-semibold text-[#111111]">{t.modeLabel}</div>
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              ...
              className={
                ...
                (mode === "personal"
                  ? "border-yellow-400/55 bg-[#F2EFE8] text-yellow-200"
                  : "border-black/10 bg-black/20 text-[#111111] hover:bg-[#F5F5F5]")
              }
            >
              {t.personal}
            </button>
          ...
          {mode === "personal" ? (
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="rounded-2xl border border-black/10 bg-black/20 p-5">
                <div className="text-lg font-bold text-[#111111]">{t.freeTitle}</div>
                <div className="mt-2 text-sm text-[#111111]">{t.freeHint}</div>
              </div>
              <div className="rounded-2xl border border-yellow-400/25 bg-[#F2EFE8] p-5">
                <div className="text-lg font-bold text-yellow-200">{t.proTitle}</div>
                <div className="mt-2 text-sm text-[#111111]">{t.proHint}</div>
              </div>
            </div>
          ) : (
            <div className="mt-6">
              <div className="text-sm font-semibold text-[#111111]">{t.pickCat}</div>
              <select
                value={bizCat}
              ...
              <div className="mt-5 rounded-2xl border border-black/10 bg-black/20 p-5">
                <div className="text-sm text-[#111111]">{t.priceLabel}</div>
                ...
                <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <div className="text-sm font-semibold text-[#111111]">{t.included}</div>
                  ...
                  <div>
                    <div className="text-sm font-semibold text-[#111111]">{t.excl}</div>
                    <ul className="mt-2 space-y-2 text-sm text-[#111111]">
                      {t.bizExcluded.map((b) => (
                        <li key={b} className="flex gap-2">
