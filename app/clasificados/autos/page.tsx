...
const CATEGORY = "autos";

const COPY = {
  es: {
    title: "Autos",
    subtitle: "Compra y vende vehículos, refacciones y servicios automotrices.",
    searchPlaceholder: "Buscar: marca, modelo, año…",
  ...
  en: {
    title: "Autos",
    subtitle: "Buy and sell vehicles, parts and auto services.",
...
export default function Page() {
  ...
  const t = COPY[lang];
  const chips = AUTOS_QUICK_CHIPS[lang];

  const listaHref = useMemo(() => buildAutosListaUrl(CATEGORY, lang), [lang]);
        ...
        <section className="mt-4 rounded-xl border border-[#C9B46A]/25 bg-[#F5F5F5] shadow-sm p-3 ring-1 ring-[#C9B46A]/10">
          ...
          <div className="mt-2 flex gap-1.5 overflow-x-auto pb-0.5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {chips.map((label) => (
              <Link
                key={label}
                href={buildAutosListaUrl(CATEGORY, lang, label)}
