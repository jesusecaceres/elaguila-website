...
import { BrNegocioStreamableVideo } from "./BrNegocioStreamableVideo";

type Slide =
  | { kind: "photo"; index: number; url: string; caption: string }
  | { kind: "video"; slot: 0 | 1 };

function buildSlides(vm: BienesRaicesNegocioPreviewVm): Slide[] {
  const m = vm.media;
...
function VideoSlide({ vm, slot }: { vm: BienesRaicesNegocioPreviewVm; slot: 0 | 1 }) {
  const m = vm.media;
