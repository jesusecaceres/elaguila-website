...
const COPY = {
  es: {
    title: "En Venta",
    subtitle: "Compra y vende local: muebles, electrónica, herramientas y más.",
    searchPlaceholder: "Buscar: sofá, celular, herramientas…",
  ...
  en: {
    title: "For Sale",
    subtitle: "Buy and sell local: furniture, electronics, tools and more.",
    searchPlaceholder: "Search: sofa, phone, tools…",
      ...
      <main className="mx-auto max-w-4xl px-4 pt-6 pb-12">
        ...
        <section className="mt-4 rounded-xl border border-[#C9B46A]/25 bg-[#F5F5F5] shadow-sm p-3 ring-1 ring-[#C9B46A]/10">
          <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-2 items-stretch">
            <div className="relative min-w-0">
              ...
              <input
                ...
                className="w-full rounded-lg border border-[#C9B46A]/30 bg-white py-2 pl-8 pr-3 text-sm text-[#111111] outline-none placeholder:text-[#111111]/70 focus:border-[#A98C2A]/60 focus:ring-1 focus:ring-[#A98C2A]/20"
                aria-label={t.searchPlaceholder}
                readOnly
                onFocus={(e) => e.target.blur()}
              />
            </div>
            <Link href={listaHref} className="flex items-center rounded-lg border border-[#C9B46A]/30 bg-white px-3 py-2 text-sm text-[#111111]/80 hover:bg-[#EFEFEF] transition sm:w-36">
          ...
          <div className="mt-2 flex gap-1.5 overflow-x-auto pb-0.5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {chips.map((label) => (
              <Link key={label} href={buildListaUrl(CATEGORY, lang, label)} className="shrink-0 rounded-full border border-[#111111]/15 bg-white px-2.5 py-1.5 text-xs font-medium text-[#111111] hover:bg-[#EFEFEF] transition">
                {label}
              </Link>
            ))}
          </div>
