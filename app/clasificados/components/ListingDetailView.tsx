...
export type ListingDetailDraft = {
  title: string;
  priceLabel: string;
  city: string;
  description: string;
...
type Props = {
  listing: ListingDetailDraft;
  previewMode?: boolean;
};

export default function ListingDetailView({ listing, previewMode = false }: Props) {
  ...
  return (
    <div className="mt-10 grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div className="lg:col-span-8">
        <div className="rounded-2xl border border-black/10 bg-[#D9D9D9]/35 backdrop-blur p-8">
          {/* Gallery */}
          {mediaSlots.length > 0 && (
            <div
          ...
          {hasProVideo && mediaSlots.length <= 1 && (
            <div className="mt-6 rounded-2xl border border-[#C9B46A]/55 bg-[#F5F5F5] backdrop-blur ring-1 ring-[#C9B46A]/25 p-6">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-sm font-semibold text-yellow-200">{t.proVideo}</div>
                  <div className="mt-1 text-xs text-[#111111]">{t.tapToPlay}</div>
                </div>
                {!showProVideo && (
                  <button
                    type="button"
      ...
      <div className="lg:col-span-4 space-y-6">
        {/* CTA buttons (Guardar, Compartir, Contactar) — disabled in preview */}
