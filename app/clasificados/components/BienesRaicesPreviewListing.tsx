...
export default function BienesRaicesPreviewListing({ listing }: BienesRaicesPreviewListingProps) {
  ...
  /**
   * NEGOCIO main hero: mobile/tablet photo-forward aspects; desktop (lg+) strong width inside preview column.
   */
  const negocioHeroOnly = (
    <div className="relative min-h-[200px] w-full min-w-0 overflow-hidden rounded-2xl border border-stone-200/80 bg-stone-100 shadow-inner aspect-[4/3] sm:min-h-[240px] sm:aspect-[5/4] lg:aspect-auto lg:min-h-0 lg:h-[min(52vw,560px)] lg:max-h-[560px] lg:min-h-[320px] lg:max-w-full lg:shrink-0 lg:shadow-sm">
  ...
  /** Tiles inside the square 2×2 wrapper: equal quadrants. */
      ...
      <div className={cx(showBusinessRail ? "p-4 sm:p-5" : "p-4 sm:p-6 lg:p-8")}>
        {showBusinessRail && listing.businessRail ? (
          <>
            ...
            <div className="mx-auto w-full max-w-[80rem] min-w-0">
              <div className="min-w-0 flex flex-col gap-4 sm:gap-5 lg:grid lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-start lg:gap-8 xl:grid-cols-[minmax(0,1fr)_24rem]">
                <div className="min-w-0 flex w-full flex-col gap-4 sm:gap-5 lg:col-start-1 lg:row-start-1 lg:min-w-0">
                  <div className="flex w-full min-w-0 flex-col gap-4 sm:gap-4 lg:grid lg:grid-cols-[minmax(0,1fr)_16rem] lg:items-start lg:gap-5">
                    <div className="flex w-full min-w-0 justify-center lg:min-w-0 lg:max-w-full lg:justify-start">{negocioHeroOnly}</div>
                    <div className="flex w-full min-w-0 justify-center lg:min-w-0 lg:max-w-full lg:justify-start">{negocioMedia2x2}</div>
                  </div>
        ...
        {/* Privado / non-negocio: hero then details stack (BR negocio uses the layout above). */}
        {!showBusinessRail ? (
