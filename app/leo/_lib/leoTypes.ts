...
/* -------------------------------------------------------------------------- */
/* LEO FINAL-02 connected action truth — Contacts + Gmail write + Calendar write */
/* Production write execution is fail-closed until FINAL-03 rollout gates pass. */
/* -------------------------------------------------------------------------- */
...
export type LeoResolvedContact = {
  email: string;
  displayName: string | null;
};
...
export type LeoContactResolutionResult = {
  ...
  /** Bounded candidate list for owner clarification — only populated when AMBIGUOUS. */
  candidates: LeoResolvedContact[];
  errorCode: string | null;
  limitations: string[];
};

/** Truthful Google read-capability classification — never assumes success. */
export type LeoGoogleReadCapabilityState =
  | "CONFIG_MISSING"
  | "TOKEN_UNAVAILABLE"
  | "READ_READY";

/**
 * Truthful Google write-capability classification.
 * WRITE_READY requires: config + token + LEO_GOOGLE_WRITE_ENABLED=true. Scope
 * possession itself cannot be proven ahead of a real provider call (Google does
 * not expose granted-scope introspection on the token endpoint LEO uses), so
 * WRITE_READY is "eligible to attempt", not "scopes proven" — SCOPE_MISSING is
 * reserved for the case a provider call itself reports insufficient scope.
 */
...
export type LeoCalendarActionProposal = {
  ...
  /** Required and must be a proven existing event for UPDATE_CALENDAR_EVENT. */
...
/** Durable-adjacent prepared proposal — the exact content the owner confirmed against. */
...
export type LeoActionExecutionState =
  | "SUCCEEDED"
  | "FAILED"
  | "DUPLICATE_REPLAY"
  | "AMBIGUOUS"
