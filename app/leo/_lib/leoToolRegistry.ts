...
export const LEO_TOOL_REGISTRY: Record<LeoToolId, LeoToolDefinition> = {
  ...
  "leo.email.inbox.read": def({
    id: "leo.email.inbox.read",
    name: "Gmail inbox read",
    description: "Read bounded recent Gmail inbox metadata for the configured owner account.",
    category: "COMMUNICATION",
    operationModes: ["READ"],
    availability: "NOT_CONFIGURED",
    requiredGovernanceAction: "READ",
    readScopes: ["gmail_inbox_metadata"],
    writeScopes: [],
    ...
    limitations: [
      ...INTERNAL_LIMITATIONS,
      "Gmail read-only — no send/modify/delete/attachments.",
      "Bounded recent messages — not global inbox totals.",
    ],
    verified: false,
    version: "13.0.0",
  }),
  "leo.email.thread.read": def({
    id: "leo.email.thread.read",
    name: "Gmail thread read",
    description: "Read bounded Gmail thread metadata for reply-direction evidence.",
    category: "COMMUNICATION",
    operationModes: ["READ"],
    availability: "NOT_CONFIGURED",
    requiredGovernanceAction: "READ",
    readScopes: ["gmail_thread_metadata"],
    writeScopes: [],
    externalSystem: "GOOGLE",
    supportsPreparation: false,
    evidenceRequirements: ["thread_id", "LEO_GOOGLE_REFRESH_TOKEN"],
    limitations: [
      ...INTERNAL_LIMITATIONS,
      "Gmail read-only — metadata only.",
    ],
    verified: false,
    version: "13.0.0",
  }),
  "leo.calendar.events.read": def({
    id: "leo.calendar.events.read",
    name: "Calendar events read",
    description: "Read bounded Google Calendar events for a requested window.",
    category: "CALENDAR",
    operationModes: ["READ"],
    availability: "NOT_CONFIGURED",
    requiredGovernanceAction: "READ",
    readScopes: ["calendar_events"],
    writeScopes: [],
    externalSystem: "GOOGLE",
    supportsPreparation: false,
    evidenceRequirements: ["LEO_GOOGLE_REFRESH_TOKEN"],
  ...
  "leo.communication.snapshot.read": def({
    id: "leo.communication.snapshot.read",
    name: "Communication snapshot",
    description: "Combine Gmail + Calendar evidence into a bounded executive snapshot.",
    category: "COMMUNICATION",
    operationModes: ["ANALYZE"],
    availability: "NOT_CONFIGURED",
    requiredGovernanceAction: "ANALYZE",
    readScopes: ["communication_snapshot"],
    writeScopes: [],
    externalSystem: "GOOGLE",
    supportsPreparation: false,
    evidenceRequirements: ["gmail_and_or_calendar"],
    limitations: [
      ...INTERNAL_LIMITATIONS,
      "PARTIAL when only one of Gmail or Calendar is available.",
      "Does not claim global inbox counts from bounded fetches.",
    ],
    verified: false,
    version: "13.0.0",
  }),
  "leo.meeting.prepare": def({
    id: "leo.meeting.prepare",
    name: "Meeting preparation",
    description:
      "Prepare a YELLOW meeting brief from calendar + related Gmail evidence — not executed.",
    category: "PREPARATION",
    operationModes: ["PREPARE"],
    availability: "NOT_CONFIGURED",
    requiredGovernanceAction: "PREPARE_DRAFT",
    readScopes: ["meeting_intelligence"],
    writeScopes: [],
    externalSystem: "GOOGLE",
    supportsPreparation: true,
    evidenceRequirements: ["next_or_selected_meeting"],
    limitations: [
      ...INTERNAL_LIMITATIONS,
      "Prepared work remains NOT_EXECUTED.",
      "Does not send invites or modify calendar.",
    ],
    verified: false,
    version: "13.0.0",
  }),
