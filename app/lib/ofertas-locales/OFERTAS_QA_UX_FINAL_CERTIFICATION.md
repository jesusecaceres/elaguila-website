...
## 21. TWO-LANE BACKEND CLOSEOUT

**Status: NOT a human production sign-off.** This section closes the two proven gaps left open at the end of Section 20 (Gate K item 38, and coupons' free-submission path) with real, source-verified backend work — no scanner file touched, no new table, no new search engine.

1. **Gap A — manual coupons join the existing searchable-item ecosystem.** New unprotected `ofertasLocalesCouponItemSync.ts` builds an `oferta_local_items` row per authored coupon by constructing an `OfertaLocalSearchableItemDraft` and handing it to the *existing, protected, unmodified* export `mapOfertaLocalSearchableItemDraftToDbInsert()` (in `ofertasLocalesAiDbMapper.ts`) — the same certified mapper AI-scanned items already go through — then overriding `id` with the coupon entry's own stable client-generated UUID. `scan_job_id` is left `null`, the pre-existing nullable marker the real `public-search` route already ignores when deciding eligibility (confirmed by reading that route's query — no `scan_job_id` filter exists there). A new, narrowly-scoped, explicitly-authorized route `app/api/ofertas-locales/coupons/sync/route.ts` upserts these rows (`onConflict: "id"`) and deactivates (`is_active: false`, never deletes) any previously-synced row whose entry was removed — giving idempotent create/edit/remove with zero duplicate rows on retry. The route defensively re-derives the product from the parent's own persisted `offer_type` and 422s if it ever resolves to an AI-included product, making it structurally impossible to invoke against a flyer listing. **No DB migration.** Client wiring: a new debounced (900ms) sync effect in `OfertasLocalesApplicationClient.tsx` calls `ensureOfertaLocalCouponRecord()` (which, per Section 20 item 4, was already written but never actually invoked — now fixed) followed by the new `syncOfertaLocalCouponItems()`, with a small ES/EN status indicator.

2. **Gap B — free coupon submission needs no Stripe.** `validateOfertaLocalSubmissionEntitlement()` (`ofertasLocalesCommercialServer.ts`) gained a third entitlement source, `"free"`, returned *before* any payment or partner-courtesy table is queried, gated solely on the server-resolved commercial product's own `amountCents === 0` — never a client-supplied flag. Because `weekly_flyer` always resolves to the $399 `interactive_flyer` product (never `amountCents === 0`), this path is structurally unreachable for the flyer lane; proven directly by calling the real function with a Supabase mock whose `.from()` throws if invoked at all (Gate L items 20–22) — the free path never touches the database, and the flyer path, given the same empty-entitlement mock, still correctly returns `commercial_entitlement_required`/402. `publish/route.ts`'s AI-review-specific validation block (scan-job + approved-source-item checks) now runs only when `entitlement.product.aiIncluded`, since that check requires a real scanned source item that manual coupon rows structurally never have; the flyer's paid lifecycle and its AI-review gate are otherwise byte-identical to Section 20.

3. **Public eligibility / moderation unchanged.** Manual coupon rows are created `review_status: "approved"`, `is_active: true` (they're owner-authored, not AI-suggested — there's nothing to moderate item-by-item), but the *actual* public gate is unchanged: `canOfertaLocalItemBePubliclyEligible()` (protected, untouched) still requires the **parent** `ofertas_locales.status === "approved"`. Called directly with a `pending_review` parent it returns `false`; with an `approved` parent, `true` (Gate L items 24–25, real function calls, not regex). The full promo flyer stays on the pre-existing `couponAssets` column/mechanism and is never item-ized.

4. **Stale-test fix (pre-existing, exposed by Section 20's catalog flip, not by this gate's own changes).** `scripts/ofertas-locales-ai-entitlement-gate-regression-audit.ts` Case B previously asserted `offerType: "coupon"` must report AI-scan-ready — true before Section 20, false ever since coupons became a genuinely free, non-AI product. Rewritten to assert the current, correct invariant instead: coupon AI readiness/persist-eligibility is `false` regardless of which way the deprecated `wantsAiSearchableSpecials` legacy flag is set, proving the canonical `isOfertaLocalAiIncludedInPackage()` helper — not the legacy flag — remains the sole source of truth. No production code changed for this fix, only the test's own outdated fixture.

5. **New regression coverage.** `scripts/ofertas-locales-gate-l-two-lane-backend-closeout-audit.ts` — 34/34 TRUE, mixing real function calls (`buildOfertaLocalCouponItemInsertRow`, `findStaleOfertaLocalCouponItemIds`, `canOfertaLocalItemBePubliclyEligible`, `validateOfertaLocalSubmissionEntitlement` against a minimal mock Supabase client) with structural source assertions where DB/network I/O made a real call impractical. Because the free-entitlement function lives in a `server-only`-guarded module, the gate runs via a small launcher (`scripts/ofertas-locales-gate-l-run.cjs`) that sets the `react-server` Node export condition on a child process — no production import boundary was weakened to make this possible. `scripts/ofertas-locales-gate-k-two-lane-final-verifier.ts` item 38 now asserts the sync mapper/route actually exist and wire the certified mapper + `scan_job_id IS NULL` marker; Gate K's pass threshold was tightened from a documented 67/68 to an unconditional **68/68 TRUE**.

6. **Full consolidated regression run (all green):** `ofertas:ai-scanner-certified-baseline-audit`, Gate H, Gate I, Gate J, Gate K (68/68), Gate L (34/34), `ai-entitlement-gate-regression-audit` (post-fix), `scan-persist-publish-separation-audit`, `gate-f-final-review-preview-handoff-regression-audit`. `git diff --check` clean. Full `npx tsc --noEmit` re-confirmed zero new errors beyond the pre-existing `e2e/` baseline noise. `npm run build`: fails at the `/dashboard` prerender step on `Missing NEXT_PUBLIC_SUPABASE_URL or NEXT_PUBLIC_SUPABASE_ANON_KEY` — this worktree has no `.env*` file at all, the failure is on an untouched page, and zero files under `app/lib/supabase/` changed in this diff, so it is a pre-existing local-environment gap, not a defect introduced by this closeout.

**Human production sign-off is still PENDING**, exactly as in Section 20 — this section documents backend correctness, not a deployment approval.
