...
export function getOfertaLocalActiveScanCopy(lang: OfertasLocalesAppLang): OfertaLocalActiveScanCopy {
  if (lang === "en") {
    return {
      ...
      cropPending: "AI found this item on the flyer, but the product clip is not ready yet.",
      cropNone: "No product clip or location available yet — use the flyer reference to review this item.",
      cropAdClipPage: "Ad clip · Page",
      listCropPending: "Location found",
  ...
  return {
    ...
    cropPending: "AI encontró este producto en el volante, pero el recorte no está listo todavía.",
    cropNone: "No hay recorte ni ubicación disponible todavía — usa la referencia del volante para revisar este producto.",
    cropAdClipPage: "Recorte del anuncio · Página",
    listCropPending: "Ubicación encontrada",
...
/** Items still requiring operator review on the current page. */
export function isOfertaLocalActiveReviewStatus(
  status: OfertaLocalItemReviewStatus | null | undefined
): boolean {
  return status == null || status === "pending" || status === "needs_review";
}

export function isOfertaLocalReviewedStatus(
  status: OfertaLocalItemReviewStatus | null | undefined
): boolean {
  return status === "approved" || status === "rejected";
}

export function partitionOfertaLocalPageReviewItems(items: OfertaLocalItemReviewViewModel[]): {
  allPageItems: OfertaLocalItemReviewViewModel[];
  activeReviewItems: OfertaLocalItemReviewViewModel[];
  reviewedItems: OfertaLocalItemReviewViewModel[];
} {
  const allPageItems = items;
  const activeReviewItems = items.filter((item) => isOfertaLocalActiveReviewStatus(item.reviewStatus));
  const reviewedItems = items.filter((item) => isOfertaLocalReviewedStatus(item.reviewStatus));
  return { allPageItems, activeReviewItems, reviewedItems };
}

export type ClipReviewDisplayRect = {
  left: number;
  top: number;
  width: number;
  height: number;
...
function isNormalizedRange(min: number, max: number): boolean {
  return min >= 0 && min <= 1 && max >= 0 && max <= 1;
}

/** Map source bbox to pixel coordinates on the rendered page surface. */
export function mapOfertaLocalSourceBboxToDisplayRect(
  bbox: Record<string, unknown> | null | undefined,
  displayWidth: number,
  displayHeight: number,
  sourceWidth?: number | null,
  sourceHeight?: number | null
): ClipReviewDisplayRect | null {
  if (!bbox || displayWidth <= 0 || displayHeight <= 0) return null;
  ...
  if ([x, y, width, height].every((n) => Number.isFinite(n)) && width > 0 && height > 0) {
    const sw = sourceWidth && sourceWidth > 0 ? sourceWidth : null;
    const sh = sourceHeight && sourceHeight > 0 ? sourceHeight : null;
    if (!sw || !sh) return null;
    const left = clamp01(x / sw) * displayWidth;
    const top = clamp01(y / sh) * displayHeight;
    const rectWidth = clamp01(width / sw) * displayWidth;
    const rectHeight = clamp01(height / sh) * displayHeight;
    return clampClipReviewDisplayRect({ left, top, width: rectWidth, height: rectHeight }, displayWidth, displayHeight);
  }
  ...
  const normalized =
    isNormalizedRange(xMin, xMax) && isNormalizedRange(yMin, yMax) && xMax <= 1.05 && yMax <= 1.05;
  if (!normalized && (!sourceWidth || !sourceHeight)) return null;

  const nxMin = normalized ? clamp01(Math.min(xMin, xMax)) : clamp01(Math.min(xMin, xMax) / (sourceWidth ?? 1));
  const nxMax = normalized ? clamp01(Math.max(xMin, xMax)) : clamp01(Math.max(xMin, xMax) / (sourceWidth ?? 1));
  const nyMin = normalized ? clamp01(Math.min(yMin, yMax)) : clamp01(Math.min(yMin, yMax) / (sourceHeight ?? 1));
  const nyMax = normalized ? clamp01(Math.max(yMin, yMax)) : clamp01(Math.max(yMin, yMax) / (sourceHeight ?? 1));

  const left = nxMin * displayWidth;
  const top = nyMin * displayHeight;
  const rectWidth = Math.max(0, (nxMax - nxMin) * displayWidth);
  const rectHeight = Math.max(0, (nyMax - nyMin) * displayHeight);
  if (rectWidth <= 0 || rectHeight <= 0) return null;
  return clampClipReviewDisplayRect({ left, top, width: rectWidth, height: rectHeight }, displayWidth, displayHeight);
}

export function clampClipReviewDisplayRect(
  rect: ClipReviewDisplayRect,
  displayWidth: number,
  displayHeight: number
): ClipReviewDisplayRect | null {
  const left = Math.max(0, Math.min(displayWidth, rect.left));
  const top = Math.max(0, Math.min(displayHeight, rect.top));
  const right = Math.max(left, Math.min(displayWidth, rect.left + rect.width));
  const bottom = Math.max(top, Math.min(displayHeight, rect.top + rect.height));
