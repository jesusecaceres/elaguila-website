...
export function mapOfertaLocalSearchableItemDraftToDbInsert(
  ...
  return {
    ...
    source_file_name: sanitizeOptionalText(item.sourceFileName, MAX_SHORT),
    source_context: sanitizeOptionalText(item.sourceContext, MAX_SHORT),
    source_bbox: item.sourceBbox ? (item.sourceBbox as Record<string, unknown>) : null,
    candidate_type: item.candidateType ?? "product_deal",
    regular_price_text: sanitizeOptionalText(item.regularPriceText, MAX_SHORT),
    coupon_title: sanitizeOptionalText(item.couponTitle, MAX_NAME),
    offer_text: sanitizeOptionalText(item.offerText, MAX_SHORT),
    terms: sanitizeOptionalText(item.terms, MAX_TEXT),
