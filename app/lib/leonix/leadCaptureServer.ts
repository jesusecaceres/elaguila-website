...
export type SaveLeonixLeadResult =
  | { ok: true; id: string }
  | {
      ok: false;
      error:
...
/** @deprecated Use SaveLeonixLeadResult */
export type SaveContactInquiryResult = SaveLeonixLeadResult;

function mergeNewsletterInterests(interests?: string, audienceType?: unknown): string {
  const parts: string[] = [];
  const aud = parseAudienceType(audienceType);
  if (aud) parts.push(`audience:${aud}`);
  const base = trimField(interests, LEAD_LIMITS.interests);
  if (base) parts.push(base);
  return parts.join("; ").slice(0, LEAD_LIMITS.interests);
}
...
export async function saveNewsletterSubscriber(
  ...
  const row = {
    ...
    city: normalizeNorCalCityForSubmit(trimField(input.city, LEAD_LIMITS.city), lang),
    zip_code: trimField(input.zipCode, LEAD_LIMITS.zipCode),
    preferred_language: parsePreferredLanguage(input.preferredLanguage),
    interests: mergeNewsletterInterests(input.interests, input.audienceType),
...
export async function saveMediaKitLead(
  ...
  const row = {
    name,
    email,
    phone: normalizePhoneForSubmit(trimField(input.phone, LEAD_LIMITS.phone)),
...
export async function saveLeonixLead(
  supabase: SupabaseClient,
  input: {
    fullName: string;
    email: string;
  ...
  const row = {
    ...
    city_area: normalizeNorCalCityForSubmit(trimField(input.cityArea, LEAD_LIMITS.cityArea), lang),
  ...
  if (error || !inserted?.id) {
    console.error("[leads] insert failed", { code: error?.code });
    return { ok: false, error: "save_failed" };
  }
...
/** @deprecated Use saveLeonixLead */
export const saveContactInquiry = saveLeonixLead;
