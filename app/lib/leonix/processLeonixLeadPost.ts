...
export async function processLeonixLeadPost(req: Request): Promise<NextResponse> {
  ...
  if (dbConfigured) {
    try {
      ...
      const result = await saveLeonixLead(supabase, payload);
      if (result.ok) {
        saved = true;
        savedId = result.id;
        ...
        const messages: Record<string, string> = {
          ...
          save_failed: lang === "en" ? "Could not save lead." : "No se pudo guardar el lead.",
        };
