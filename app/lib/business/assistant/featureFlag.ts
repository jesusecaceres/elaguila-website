/**
 * Program 7 — Contextual Business Concierge Assistant feature flags.
...
async function fetchAssistantFlagRow(flagKey: string): Promise<BusinessIdentityFlagRow | null> {
...
export async function resolveAssistantFlagTier(userId: string | null): Promise<ResolvedFlagTier> {
  const row = await fetchAssistantFlagRow(BUSINESS_CONTEXTUAL_ASSISTANT_FLAG_KEY);
  return computeFlagTier(row, userId);
}

export async function isAssistantEnabled(): Promise<boolean> {
  const tier = await resolveAssistantFlagTier(null);
  return tier === "global";
}
