...
async function applyBrActivatePending(row: BrListingRowForPayment): Promise<BrLifecycleMutationResult> {
  ...
  // 2026-09 category closeout (recovery port 2026-09-25) — the atomic RPC deliberately tolerates "no
  // entitlement yet" for a main row (the paid Stripe / Revenue OS fulfillment activates via
  // `tryActivateBrListingAfterPayment`, which does NOT come through here). This owner-facing route
  // therefore must not: without a check, an owner's own pending main row could be brought live for free.
  // Outside the dev/QA payment bypass, a main (or legacy null-role) row needs an ACTIVE entitlement for
  // EITHER Bienes base package — Full (`br_agent_monthly`) or Quick/BASE (`br_agent_quick_monthly`) —
  // resolved via `businessBasePackageKeys`, never a hardcoded Full-only key (that would block paid Quick
  // customers). Children are already gated by the active parent + pack capacity below.
  if (row.inventory_role !== "inventory_property" && brPublishPaymentRequired("negocio")) {
    let hasActiveBaseEntitlement = false;
    for (const packageKey of businessBasePackageKeys("bienes-raices")) {
      ...
      if (ent.get(row.id)?.status === "active") {
        hasActiveBaseEntitlement = true;
        break;
      }
    }
    if (!hasActiveBaseEntitlement) {
      return { ok: false, error: BR_LIFECYCLE_TRANSITION_NOT_ALLOWED_ERROR };
    }
  }
