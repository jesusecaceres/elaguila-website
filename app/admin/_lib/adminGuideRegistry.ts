...
export const ADMIN_GUIDE_ENTRIES: AdminGuideEntry[] = [
  ...
  },
  {
    ...
    purpose: "Operations for the Viajes/travel marketplace vertical — staged business offer listings awaiting review.",
    useWhen: "You need to review or moderate a travel business offer.",
    commonTasks: ["Review a staged Viajes business offer", "Approve, request changes, or reject a submission"],
    howTo: ["Open Viajes from the sidebar", "Open Business Offers for the real moderation queue"],
    permissionNote: "Available to every non-sales-rep role.",
    relatedAdminRoutes: ["/admin/clasificados/viajes/business-offers", "/admin/workspace/clasificados/travel"],
    actionLevel: "yellow",
    keywords: ["viajes", "travel listing", "travel business offer"],
    canonicalEntity: "viajes_staged_listings",
    audience: "staff",
    leoSafeReadSource: true,
    notes: "A second Viajes-adjacent page exists at /admin/workspace/clasificados/travel — confirmed a real, functioning staged-listings queue, not a broken duplicate; this is the primary-nav entry point. Launch Truth Doctrine pass: Affiliate Cards, Campaigns, Editorial, and Settings sub-pages were illustrative/mock-data tools with no working save path and are no longer linked from this page — Business Offers moderation is the one real, working Viajes capability today.",
  },
  {
  ...
  // ============================================================= PEOPLE =============================================================
  {
    ...
    useWhen: "You need to sign in to Admin, or you forgot your password. Use normal login (with password reset if needed) unless Supabase Auth itself is unavailable — bootstrap exists only for that emergency case.",
    commonTasks: ["Sign in day-to-day with your Leonix staff/admin email and password", "Reset a forgotten password via 'Forgot password?'", "Use owner bootstrap only when normal login cannot be used", "Log out to clear your session"],
    howTo: [
      ...
      "Forgot your password? Use the 'Forgot password?' link under the Staff / Team login form, enter your team email, and check your inbox for a recovery link — the same message shows whether or not an account exists for that email, by design",
      "Opening the recovery email link takes you to a dedicated admin password-reset screen (never the customer dashboard) — set a new password there, then use 'Return to Staff / Team login' to sign in normally",
    ...
    permissionNote: "Normal login: any active Team Roster member, scoped to their own role's capabilities. Forgot password/reset: available to anyone who can prove control of a Supabase Auth email (staff or not) — it changes only that Supabase Auth password, never roster membership, role, or permissions. Owner bootstrap: broad emergency owner-level read access intended for recovery, but it can never perform a Business Concierge write or any write that requires a real staff identity — those always require a real per-person login.",
    relatedAdminRoutes: ["/admin/team/roster", "/admin/team/users/new", "/admin/team/my-profile"],
    actionLevel: "yellow",
    failureGuidance: "\"Your team account is inactive\" or \"This account is not authorized for admin access\" means the Supabase Auth credentials were correct but no active Team Roster row exists for that email — ask an owner_admin to check Team Roster. \"We couldn't open your recovery link\" means the reset link was already used, expired, or malformed — request a new one from 'Forgot password?'. \"Owner bootstrap login is not configured on this server\" means the emergency recovery path itself is not available in this environment — contact an owner_admin, do not keep retrying the shared password.",
    keywords: ["login", "sign in", "admin login", "bootstrap", "owner bootstrap", "emergency access", "locked out", "shared password", "break glass", "break-glass", "normal login", "daily login", "forgot password", "reset password", "password recovery", "recover password"],
    canonicalEntity: "admin_team_members (Supabase Auth-backed), owner bootstrap session (emergency-only, no roster identity)",
    audience: "both",
    leoSafeReadSource: false,
    notes: "Owner bootstrap is intentionally broad for emergency recovery reads, but by design cannot write to Business Concierge or any surface that requires an attributable staff identity (see requireStaffWorkspaceWriteAccess() — bootstrap is always denied there). If normal login is broken for the owner specifically, use bootstrap to reach Team Roster / Create staff login and repair or (re)create the owner's own real account rather than continuing to operate day-to-day as bootstrap. Forgot-password (/admin/login/forgot -> /admin/login/reset) reuses the exact same Supabase resetPasswordForEmail/callback/updateUser primitives the customer site uses — it is deliberately not a second recovery engine, and it is completely unrelated to and never touches owner bootstrap.",
  },
  {
    id: "business-360",
    ...
    purpose: "The canonical business identity list and the Business 360 detail page — one place to see a business's status, contacts, follow-ups, notes, connected records, and Commercial Benefits (package entitlements, promo redemptions, and payment records tied to its verified connected listings).",
    useWhen: "You need the full picture on one client business, want to see who needs follow-up, or need to check/manage what entitlements or promo codes are attached to their listings.",
    commonTasks: ["Find a business and open its 360 view", "Check who's overdue or waiting on the owner", "Add a note or schedule a follow-up", "Check a business's entitlements, promo redemptions, or payment history, and jump straight into the workspace to manage one"],
    howTo: ["Open Businesses", "Search by name or use the follow-up filters", "Open a business row for the full 360 detail", "Scroll to Commercial Benefits for entitlement/promo/payment detail, or use its \"Manage...\" links to act on one"],
    ...
    relatedAdminRoutes: ["/admin/ops", "/admin/workspace/package-entitlements", "/admin/workspace/promo-codes"],
    actionLevel: "yellow",
    keywords: ["business concierge", "business 360", "client", "customer follow up", "who needs follow up", "commercial benefits", "entitlements", "promo codes", "grant source"],
    canonicalEntity: "businesses",
    audience: "both",
    leoSafeReadSource: true,
  },
  {
    // Gate 15 (HELP-004) — business_proposals had no Admin Guide entry and no Company Search
    // coverage; the tab exists and is real (create/list/detail on the Business 360 page) but was
    // undiscoverable except by already knowing to scroll to the Proposals tab.
    id: "business-proposals",
    title: "Business Proposals / Contracts",
    domain: "people",
    // No standalone route exists — this lives inside each business's own Business 360 page as a
    // tab, so the guide's "Open" link goes to the businesses list (pick a business, then its
    // Proposals tab), never a literal unresolved `[businessId]` path.
    route: "/admin/businesses",
    purpose: "Create and track proposals/contracts for one business, from the Proposals tab on its Business 360 page.",
    useWhen: "You need to draft, send, or check the status of a proposal or contract for a specific business.",
    commonTasks: ["Create a new proposal for a business", "Check a proposal's current status", "Review proposal history for a business"],
    howTo: ["Open the business's Business 360 page", "Open the Proposals tab", "Create or review a proposal there"],
    permissionNote: "Same access as the Business 360 page itself — scoped for sales reps to their own assigned businesses.",
    relatedAdminRoutes: ["/admin/businesses"],
    actionLevel: "yellow",
    keywords: ["proposal", "contract", "business proposal", "business contract", "quote for business"],
    canonicalEntity: "business_proposals",
    audience: "both",
    leoSafeReadSource: false,
    notes: "Not yet covered by Company Search (/admin/ops) — a proposal cannot currently be found by search, only by opening its business first.",
  },
  {
    id: "team-hub",
    ...
    relatedAdminRoutes: ["/admin/team/roster", "/admin/team/executive-hub", "/admin/team/users/new", "/admin/digital-contact/doorbell"],
    actionLevel: "green",
  ...
  },
  {
    ...
    relatedAdminRoutes: ["/admin/team/roster", "/admin/team/my-profile", "/admin/digital-contact/doorbell"],
  ...
  {
    id: "virtual-front-desk-doorbell",
    title: "Virtual Front Desk / Digital Doorbell",
    domain: "people",
    route: "/admin/digital-contact/doorbell",
    purpose: "A visitor at the public /visitanos page can request a live video call. This page enrolls YOUR device (push notification, Samsung/Android-first) so you're alerted and can join. The visitor's fallback contact methods (WhatsApp, phone, text, email) always come from that staff member's Executive Hub profile — there is no separate 'front desk contact info' to configure.",
    useWhen: "A staff member who covers visitor calls needs their device to actually ring/notify, or you need to explain to a new manager how a walk-in visitor reaches a real person.",
    commonTasks: ["Enable push notifications on this device", "Set a temporary AVAILABLE / BUSY / AWAY presence status", "Open the live visitor page to see what a visitor sees", "Confirm which contact channels a visitor falls back to if video isn't available"],
    howTo: ["Open Virtual Front Desk from the sidebar", "Click enable notifications and grant the browser/OS permission prompt", "Optionally open Presence to set a short AVAILABLE/BUSY/AWAY window", "Open /visitanos in a new tab to see the live visitor experience"],
    statuses: [{ label: "within office hours (9am–5pm Pacific)", meaning: "The video request button is active for the visitor." }, { label: "outside office hours", meaning: "The video option is shown as informational only, not active — the visitor still sees WhatsApp/phone/text/email fallback options." }],
    permissionNote: "Any staff member with general Admin nav access can enroll their own device and set their own presence — this is a personal action, not owner-only, the same as My Profile.",
    relatedAdminRoutes: ["/admin/digital-contact/presence", "/admin/team/executive-hub", "/admin/team/my-profile", "/admin/system-health"],
    relatedPublicRoutes: ["/visitanos", "/contact/[slug]"],
    actionLevel: "yellow",
    failureGuidance: "If push notifications never arrive, first check System Health's 'Virtual Front Desk (visitor doorbell)' row — if it says NOT_CONFIGURED, the missing piece (push keys or Daily) is named there and is an environment/provider setup issue, not something fixable from this page. If System Health shows it configured, next confirm this page shows notifications enabled on the exact device that should ring, and that the browser/OS didn't silently deny the permission. If a visitor sees no fallback contact options at all, check that the relevant staff member's Executive Hub profile actually has WhatsApp/phone/email filled in — this page cannot fix that, only Executive Hub can. A static Google Meet / Microsoft Teams / FaceTime link on a profile is a secondary, emergency fallback only — it is never the primary doorbell path and does not by itself prove anyone is available. Escalation: if System Health shows configured but visitors still report no response, that's a provider-runtime issue (Daily/push service itself), not a config gap — escalate to whoever holds those provider accounts rather than treating it as an Admin bug.",
    keywords: ["virtual front desk", "digital doorbell", "visitanos", "visitor video call", "front desk", "doorbell notifications", "who rings staff", "after hours visitor", "visitor fallback contact"],
    canonicalEntity: "digital_contact_devices",
    audience: "both",
    leoSafeReadSource: true,
    notes: "System dependencies: Daily (DAILY_API_KEY, video) and Web Push/VAPID (WEB_PUSH_VAPID_PUBLIC_KEY/WEB_PUSH_VAPID_PRIVATE_KEY, notifications) — both surfaced as one config-presence-only System Health component ('Virtual Front Desk (visitor doorbell)', added 2026-09-14). No outbound call is made to either provider by that check; it only reports whether the required environment variables are set. Public impact: this system is the only path from a real, in-person office visitor to a staff member — its public half (/visitanos) always shows WhatsApp/phone/text/email regardless of this component's state, so a config gap degrades to slower fallback contact, not a dead end.",
  },
  {
    id: "virtual-front-desk-presence",
    title: "Executive Presence (Temporary Availability)",
    domain: "people",
    route: "/admin/digital-contact/presence",
    purpose: "Set a short-lived AVAILABLE / BUSY / AWAY status that controls Human Connection video eligibility on your public contact page and the visitor front desk. Every status expires — this is not a permanent schedule.",
    useWhen: "You're about to be reachable (or unreachable) for live video for a limited window and want that reflected truthfully to visitors.",
    commonTasks: ["Set AVAILABLE for the next block of time", "Set BUSY or AWAY", "Confirm a status has expired"],
    howTo: ["Open Presence (linked from Virtual Front Desk)", "Choose a status and duration", "Save — the status auto-expires, it is never left on indefinitely"],
    permissionNote: "Any staff member with a linked Executive Hub profile can set their own presence.",
    relatedAdminRoutes: ["/admin/digital-contact/doorbell", "/admin/team/executive-hub"],
    relatedPublicRoutes: ["/visitanos", "/contact/[slug]"],
    actionLevel: "green",
    failureGuidance: "Setting AVAILABLE here does not by itself guarantee a video option appears — video also requires a configured provider and the profile's allowVideo setting in Executive Hub.",
    keywords: ["presence", "available busy away", "temporary availability", "video eligibility"],
    canonicalEntity: "executives",
    audience: "staff",
    leoSafeReadSource: true,
  },
  ...
  },
  {
    ...
    purpose: "A checklist-style page reviewing whether major Admin areas resolve through the shared bilingual (EN/ES) dictionary, plus one genuinely live check: whether the EN and ES dictionary entries have the same keys.",
    useWhen: "You want a status overview of bilingual coverage across Admin chrome, or want to confirm the EN/ES string dictionaries haven't drifted out of key parity.",
    commonTasks: ["Review the language coverage checklist", "Check the live dictionary key-coverage numbers"],
    howTo: ["Open Language Audit", "The top section (\"Dictionary key coverage\") is live and recomputed every load; the table below it is a static per-section checklist"],
    permissionNote: "Requires site-settings permission.",
    actionLevel: "green",
    failureGuidance: "The per-section checklist table cannot detect a real missing-translation gap by construction — it checks chrome resolution, not individual field-level blank EN/ES content. Admin's operator-facing chrome is also English-only today by product decision (getAdminLang() always returns \"en\") — the ES dictionary exists but is dormant, not a live language switch.",
  ...
  // ============================================================= SYSTEM =============================================================
  {
    ...
    purpose: "Real, detectable health for Supabase, the audit pipeline, team roster data, whether AI providers (OpenAI moderation, AI Gateway) are configured, whether Stripe/email/SMS are configured and (for Stripe) actually working — based on recent webhook history first, falling back to one safe, read-only live API check when webhook history alone can't tell a dead key apart from no recent traffic — and whether the Virtual Front Desk's video (Daily) and push-notification (Web Push/VAPID) dependencies are configured.",
  ...
  // ======================================================= STAFF OPERATING SYSTEM (2026-09-16) =======================================================
  // Day-in-the-life entries for the Business Concierge staff home (/admin/businesses). Every
  // route below was confirmed to have a page.tsx before being added; none invents a tool.
  {
    id: "create-for-client",
    title: "Create for Client (Assisted Publishing)",
    domain: "people",
    route: "/admin/businesses/create-for-client",
    purpose: "Open the REAL category application (Servicios, Restaurantes, Autos, Empleos, Rentas, En Venta, Viajes, Comida Local, Ofertas Locales…) on the client's behalf, from the client's confirmed Business Identity. There is no separate staff form — the same application the public uses is what you fill in, so every rule, price, and review step stays identical.",
    useWhen: "A client wants an ad or listing published and you are doing it with them (in person, on the phone, or on their behalf with their authorization).",
    commonTasks: ["Pick the business, then the category — the launcher hands you into the real application", "Servicios: contact fields prefill from Business Identity (an existing draft is never overwritten)", "Finish the application, choose the package at the checkpoint, and submit like any advertiser"],
    howTo: [
      "From the staff home, CLIENT WORK → Create Ad / Listing (or from a business page → Prepare for client → Create Ad / Listing)",
      "Confirm WHO (the business) and WHAT (the category); read the custody note",
      "The real category application opens; complete it with the client and submit",
      "Payment and review follow the category's normal path (Stripe checkout or a manually cleared offline payment recorded by the owner)",
    ],
    permissionNote: "Any Sales Workspace role (sales_rep, sales_manager, super_admin) may launch it. The listing is created under the SITE session that submits the form — the client's own login is the right custody; a shared staff site login is not.",
    relatedAdminRoutes: ["/admin/businesses", "/admin/businesses/managed", "/admin/workspace/payment-tracker/manual-payment"],
    relatedPublicRoutes: ["/publicar"],
    actionLevel: "yellow",
    failureGuidance: "If the launcher sends you to the category chooser instead of the form, the deep link key was unknown — pick the category again from the chooser. If Servicios shows an existing draft instead of the client's data, that draft was already in this browser: finish or clear it first, then relaunch. Nothing here marks anything as paid.",
    keywords: ["create for client", "create ad for client", "assisted publishing", "publish for client", "category application", "create listing", "crear anuncio", "servicios prefill"],
    canonicalEntity: "listings",
    audience: "staff",
    leoSafeReadSource: true,
  },
  {
    id: "business-research-to-truth",
    title: "Research → Human Review → Canonical Truth (Business Book)",
    domain: "people",
    route: "/admin/businesses",
    purpose: "How Leonix researches a business before a visit and how that research becomes truth. Real sources only: Google Places (when the key is configured), one bounded read of the business's own linked website, source links and photos/files staff captured in the field, and an AI briefing DRAFT synthesized from those. A draft never becomes a fact until a manager reviews it and promotes items into the Business Book (business_facts, source class ai_inference).",
    useWhen: "You are preparing for a first visit or a meeting and want what Leonix already knows, or you need to confirm something the AI draft claimed.",
    commonTasks: ["Capture source links and photos in the Field Agent", "Run research (manager+) and read the draft", "Promote confirmed items to the Business Book, reject the rest", "Ask the owner to confirm an unknown during the visit"],
    howTo: [
      "Open the business → the Prospect preparation strip → 1 · Research",
      "Add sources & files in the Field Agent if the business has none yet",
      "Run research (sales_manager / super_admin); the draft appears under Discover",
      "Review the draft: promote strengths/opportunities/unknowns/contradictions — each lands in the Business Book as ai_inference, staff-only",
      "Confirm facts with the owner; confirmed facts are what Business Profile, Creative Studio, and Create for Client build from",
    ],
    statuses: [
      { label: "draft", meaning: "AI output awaiting human review — not truth." },
      { label: "staff_reviewed", meaning: "A manager read it; items may now be promoted or rejected." },
      { label: "partially_promoted / fully_promoted", meaning: "Some / all items were promoted into the Business Book as ai_inference facts." },
      { label: "not configured", meaning: "Google Places or the AI provider key is absent in this environment — research is skipped truthfully, never faked." },
    ],
    permissionNote: "Every role can view Discover and capture sources/files. Running AI research, reviewing, and promoting a briefing are sales_manager / super_admin actions.",
    relatedAdminRoutes: ["/admin/field", "/admin/businesses"],
    actionLevel: "yellow",
    failureGuidance: "\"Consent required\" means source_research / ai_research consent was not recorded for this business — record it in the field before running. A run with zero sources and no Places key produces nothing: add a website link or a photo first. Never present a draft claim to a client as fact.",
    keywords: ["research a business", "investigar", "google places", "website research", "ai briefing", "briefing draft", "promote to book", "living book", "business book", "canonical truth", "before a visit"],
    canonicalEntity: "business_facts",
    audience: "staff",
    leoSafeReadSource: true,
  },
  {
    id: "business-profile-pipeline",
    title: "Business Profile — Staff Build → Preview → Sale → Release → Client Claim (Leonix Managed)",
    domain: "people",
    route: "/admin/businesses/managed",
    purpose: "The staff-built Leonix Business Profile pipeline and its inventory view. Staff prepare a prospect's public profile before they own it, preview it privately on a phone, record the package/sale (manager+), release it through the existing ownership-claim link, and the client claims the SAME business record; only the entitled owner can publish it to /perfil/[slug].",
    useWhen: "You want to show a prospect what their Leonix presence would look like, or you need to see which staff-built profiles are drafts, awaiting payment, ready, published, or waiting on a claim.",
    commonTasks: ["Build/edit a profile draft for a prospect", "Open the private preview on your phone", "Record a paid/complimentary package (manager+) so the profile becomes eligible", "Release to client (ownership claim link)", "Track drafts / awaiting payment / ready / published / claim pending"],
    howTo: [
      "From a business page → Prepare for client → Business Profile (or staff home → Create Business Profile for a brand-new prospect)",
      "Save the draft, then open Private preview",
      "When the client buys: a manager records the entitlement (or the owner clears an offline payment) — the state becomes eligible; nothing is ever marked paid by hand",
      "Release to client → the client claims the business with their own login and publishes from their dashboard",
      "Use Leonix Managed to see every staff-built profile by state",
    ],
    statuses: [
      { label: "draft", meaning: "Staff-built, not public." },
      { label: "awaiting payment (not eligible)", meaning: "No live entitlement yet — publishing is blocked by the database, not by the UI." },
      { label: "ready to publish (eligible)", meaning: "A live entitlement exists; the owner can publish." },
      { label: "published", meaning: "Live at /perfil/[slug]." },
      { label: "claim pending", meaning: "Released; the client has not completed the ownership claim." },
    ],
    permissionNote: "Every Sales Workspace role can build, save, preview, and release. Granting the profile entitlement (recording the sale) is sales_manager / super_admin only.",
    relatedAdminRoutes: ["/admin/businesses", "/admin/workspace/package-entitlements", "/admin/workspace/payment-tracker/manual-payment"],
    relatedPublicRoutes: ["/perfil/[slug]"],
    actionLevel: "yellow",
    failureGuidance: "If a client cannot publish, check Leonix Managed: 'awaiting payment' means no active entitlement — record the package (manager) or clear the offline payment (owner). If the client says the claim link failed, generate a fresh ownership claim from the business page; the claim engine is the same one used for every staff-created business.",
    keywords: ["business profile", "perfil de negocio", "leonix managed", "gestionado por leonix", "profile preview", "release to client", "claim pending", "awaiting payment", "entitlement", "sales pipeline"],
    canonicalEntity: "business_profiles",
    audience: "staff",
    leoSafeReadSource: true,
  },
  {
    id: "manual-payment",
    title: "Manual / Offline Payment (cash, check, Zelle, ACH, money order)",
    domain: "revenue",
    route: "/admin/workspace/payment-tracker/manual-payment",
    purpose: "Record a payment that did not go through Stripe. The record is created as pending verification; once the money is actually confirmed it is marked cleared, and only then do the paid package entitlements activate through the same canonical entitlement writer Stripe uses. No fake Stripe record, no hand-set 'paid' flag.",
    useWhen: "A client paid you in cash, by check, Zelle, ACH, or money order for a package and the listing or profile must become entitled.",
    commonTasks: ["Record the offline payment against the listing/package", "Verify it as cleared once the funds are real", "Confirm the entitlement appeared in Package Entitlements"],
    howTo: [
      "Open Payment Tracker → Manual payment",
      "Choose the method, amount, and the listing/package it pays for; save — status is pending_verification",
      "When the money is confirmed, verify cleared — entitlements activate (grant source manual_cleared_payment)",
      "Check Package Entitlements / the business's Commercial Benefits to confirm",
    ],
    statuses: [
      { label: "pending_verification", meaning: "Recorded, money not yet confirmed — grants nothing." },
      { label: "cleared", meaning: "Confirmed; entitlements activated." },
    ],
    permissionNote: "Viewing Payment Tracker needs owner access or the can_view_payments roster permission. Recording or clearing an offline payment is a revenue-protected write: the owner (super_admin) signed in with a real staff login — not sales_manager, not sales_rep, and not the shared bootstrap password.",
    relatedAdminRoutes: ["/admin/workspace/payment-tracker", "/admin/workspace/package-entitlements", "/admin/businesses/managed"],
    actionLevel: "red",
    failureGuidance: "A 403 on save means the signed-in identity is not a super_admin staff login (bootstrap is rejected by design). If the entitlement does not appear after clearing, check Package Entitlements for the listing id — the writer is the same as Stripe's, so a missing row is a data problem to escalate, not something to patch by hand.",
    keywords: ["offline payment", "manual payment", "cash payment", "zelle", "check", "ach", "money order", "pago manual", "pago en efectivo", "record a payment", "pending verification", "cleared"],
    canonicalEntity: "leonix_payment_records",
    audience: "owner",
    leoSafeReadSource: true,
  },
