...
export const ADMIN_GUIDE_ENTRIES: AdminGuideEntry[] = [
  ...
  },
  {
    ...
    relatedAdminRoutes: ["/admin/team/users/new", "/admin/team/executive-hub"],
    actionLevel: "red",
    failureGuidance: "If a new hire also needs a public contact page (photo, title, QR code, vCard), creating their login here does NOT create that — open Executive Hub separately to add it.",
  ...
  {
    id: "executive-hub",
    title: "Executive Hub (Staff Contact Profile)",
    domain: "people",
    route: "/admin/team/executive-hub",
    purpose: "Executive Hub manages a staff member's PUBLIC CONTACT PROFILE — photo, title, theme, working hours, and publish status for their public contact page. It is a completely separate system from Team Roster: there is no automatic link between a staff login and a contact profile today, and creating one does not create the other.",
    useWhen: "A staff member needs a public contact page (e.g. for a business card, QR code, or website listing), or an existing profile needs updating.",
    commonTasks: ["Create a new executive contact profile", "Edit photo, title, or working hours", "Publish, suspend, or preview a profile"],
    howTo: ["Open Executive Hub", "Use 'New' to create a profile, or open an existing one to edit", "Use Preview to see exactly what the public page will show before publishing"],
    ...
    permissionNote: "owner_admin only — a staff member cannot currently edit their own contact profile; this is a known gap, not a bug (see failureGuidance).",
    relatedAdminRoutes: ["/admin/team/roster"],
    relatedPublicRoutes: ["/contact/[slug]"],
    actionLevel: "red",
    failureGuidance: "Staff self-editing their own profile is not built yet — it needs its own authorization work beyond simple reuse of this owner-only editor. If a staff member needs a change, the owner (or another owner_admin) must make it here today.",
    keywords: ["staff contact", "contact page", "vcard", "qr code", "business card page", "public profile", "contact slug", "update staff contact page"],
    canonicalEntity: "executives",
    audience: "owner",
    leoSafeReadSource: true,
    notes: "QR code, vCard download, and the public /contact/[slug] page are all real and already built (DigitalContactQrCode.tsx, the vcf API route, digitalContactVCard.ts).",
  },
  {
    id: "users",
    title: "Users",
