 ...
 *
 * No I/O here. The one import is the checkout guard's package-key set (that module is not `server-only` and is
 * tsx-importable); every importer of this policy is server code.
 */
...
/**
 * Base packages whose entitlement lapse blocks Admin reactivation. DERIVED from the checkout guard's
 * REVENUE_BASE_ENTITLEMENT_GUARD_PACKAGE_KEYS (never a hand-maintained copy), so both the PRO keys
 * (`*_base_monthly`, `autos_dealer_monthly`, `br_agent_monthly`) and the BASE keys (`*_quick_monthly`) govern:
 * a canceled / expired BASE subscription cannot be laundered live by Admin Restore any more than a PRO one.
 */
export const ADMIN_BASE_ENTITLEMENT_PACKAGE_KEYS: readonly string[] = [...REVENUE_BASE_ENTITLEMENT_GUARD_PACKAGE_KEYS];
