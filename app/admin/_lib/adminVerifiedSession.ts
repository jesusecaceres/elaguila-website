...
/**
 * Identity-verified staff session for the general admin gate (`requireLeonixAdminCookie`), which is always followed
 * by the roster permission check (`requireLeonixAdminPermission`). Same fail-closed identity proof as above, but a
 * verified roster member whose role is outside the Sales workspace roles (e.g. a content role) still passes here —
 * their capabilities are decided by the permission check, not by this identity gate.
 */
export async function isIdentityVerifiedAdminSession(jar: CookieJar): Promise<boolean> {
  try {
    const access = await resolveSalesWorkspaceAccess(jar);
    return access.ok === true || (access.ok === false && access.reason === "role_not_permitted");
  } catch {
    return false;
  }
}
