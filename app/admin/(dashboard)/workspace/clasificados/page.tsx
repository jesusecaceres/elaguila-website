...
export default async function AdminClasificadosWorkspacePage(props: PageProps) {
  ...
  return (
    <>
      ...
      {showOverviewSections ? (
        <>
          <ClasificadosCategoryCommandCenter registry={registry} lang={lang} />
          <ClasificadosCategoryUtilitiesCollapsible lang={lang} />
          <AdminSectionCard title={m("clasificados.globalSearchTitle")} subtitle={m("clasificados.globalSearchSub")}>
            <form className="flex flex-col gap-3 sm:flex-row sm:items-end" method="get" data-testid="clasificados-global-search">
              <input
                name="q"
          ...
          <ClasificadosCategoryOpsAuditLazy lang={lang} />
          <AdminSectionCard title={m("clasificados.envSectionTitle")} subtitle={m("clasificados.envSectionSubtitle")}>
            <details className="rounded-2xl border border-[#E8DFD0]/70 bg-[#FAF7F2]/80 p-3" data-testid="clasificados-moderation-reference">
              <summary className="cursor-pointer text-sm font-semibold text-[#3D3629]">
                Open moderation reference (En Venta contract fields)
              </summary>
              <div className="mt-3">
