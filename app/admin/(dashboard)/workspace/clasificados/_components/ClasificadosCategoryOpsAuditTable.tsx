...
export function ClasificadosCategoryOpsAuditTable({
  rows,
  lang,
}: {
  rows: ClasificadosCategoryOpsAuditRow[];
  lang: AdminLang;
}) {
  const m = adminMessages(lang);

  return (
    <div className="overflow-x-auto" data-testid="clasificados-ops-audit-table">
      <table className="min-w-[1040px] w-full border-collapse text-left text-xs text-[#3D3428]">
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.slug} className="border-b border-[#E8DFD0]/60 align-top odd:bg-[#FFFCF7] even:bg-[#F8F0E3]">
