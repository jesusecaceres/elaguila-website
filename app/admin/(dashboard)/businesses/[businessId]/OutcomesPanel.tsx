    ...
    <div className="mt-6 rounded-lg border border-[color:var(--lx-border)] bg-[color:var(--lx-card)] p-4">
      ...
      >
        <h3 className="text-sm font-bold text-[color:var(--lx-text)]">
          Program 7 — Outcomes ({outcomes.length})
        </h3>
      ...
      {expanded && (
        <div className="mt-3 space-y-2">
          {outcomes.length === 0 ? (
            <p className="text-xs text-[color:var(--lx-text-muted)]">No outcomes recorded yet.</p>
          ) : (
            outcomes.map((o) => (
              <div key={o.id} className="rounded border border-[color:var(--lx-border)] p-2 text-xs">
                <div className="font-semibold">{o.metricLabelEn}</div>
                <div className="text-[color:var(--lx-text-muted)]">
