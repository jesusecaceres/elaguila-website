-- Recursos Build 03C — Certification import preflight (READ-ONLY)
...
SELECT
  (SELECT count(*) FROM public.community_resource_candidate_reviews
  ...
  'youth-space'
     )) AS existing_ready_candidate_review_count,
  (SELECT count(*) FROM public.community_resource_candidate_reviews
     WHERE candidate_id IN (
  '2-1-1-bay-area',
  'asian-americans-for-community-involvement-aaci-aaci-asian-women-s-home',
  ...
  'working-partnerships-usa-union-community-resources',
  'youth-space'
     ) AND promoted_resource_id IS NOT NULL) AS existing_promoted_links;
